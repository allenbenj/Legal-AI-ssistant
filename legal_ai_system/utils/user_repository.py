...
class UserRepository:
    ...
    async def add_user_async(self, user: AuthUser) -> None:
        self.logger.info(
            ...
            parameters={"username": user.username, "user_id": user.user_id},
        )
        try:
            ...
            self.logger.info(
                "User added successfully.", parameters={"user_id": user.user_id}
            )
        except (
            Exception
        ) as e:  # Catch specific asyncpg.UniqueViolationError if possible
    ...
    async def get_user_by_username_async(self, username: str) -> Optional[AuthUser]:
        self.logger.debug(
            "Fetching user by username.", parameters={"username": username}
        )
        try:
    ...
    async def update_user_auth_status_async(
        ...
        self.logger.info(
            ...
            parameters={"user_id": user_id, "failed_attempts": failed_attempts},
        )
        try:
            async with self.transaction_manager.transaction() as conn:
                await conn.execute(
            ...
            self.logger.info(
                "User auth status updated.", parameters={"user_id": user_id}
            )
        except Exception as e:
            self.logger.error("Failed to update user auth status.", exception=e)
            raise DatabaseError(
                f"Error updating auth status for user {user_id}", cause=e
            )

    @detailed_log_function(LogCategory.DATABASE)
    async def create_session_async(
        self, session_token: str, session_data: Dict[str, Any]
    ) -> None:
        self.logger.info(
            "Creating persistent session.",
            parameters={
                "token_preview": session_token[:8] + "...",
        ...
        try:
            ...
            self.logger.info(
                ...
                parameters={"token_preview": session_token[:8] + "..."},
            )
        except Exception as e:
            self.logger.error("Failed to create persistent session.", exception=e)
            raise DatabaseError(
                f"Error creating session for user {session_data.get('user_id')}",
                cause=e,
            )
    ...
    async def get_session_async(self, session_token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            raise DatabaseError(
                f"Error fetching session token {session_token[:8]}...", cause=e
            )

    @detailed_log_function(LogCategory.DATABASE)
    async def invalidate_session_async(self, session_token: str) -> None:
        self.logger.info(
            "Invalidating persistent session.",
            parameters={"token_preview": session_token[:8] + "..."},
        )
        try:
            async with self.transaction_manager.transaction() as conn:
                await conn.execute(
                    "UPDATE user_sessions_persistent SET is_valid = FALSE WHERE session_token = $1",
                    session_token,
                )
            self.logger.info(
                "Persistent session invalidated.",
                parameters={"token_preview": session_token[:8] + "..."},
            )
        except Exception as e:
            self.logger.error("Failed to invalidate persistent session.", exception=e)
            raise DatabaseError(
                f"Error invalidating session token {session_token[:8]}...", cause=e
            )
    ...
    async def add_audit_logs_batch_async(self, logs: List[AuditLogEntry]) -> None:
        ...
        try:
            ...
            async with (
                self.transaction_manager.transaction() as conn
            ...
            self.logger.info(
                "Batch audit logs added successfully.", parameters={"count": len(logs)}
            )
        except Exception as e:
    ...
    def _row_to_auth_user(
        self, row: Optional[asyncpg.Record]
    ) -> Optional[AuthUser]:  # Changed type hint
        if not row:
            return None
        return AuthUser(
            user_id=row["user_id"],
