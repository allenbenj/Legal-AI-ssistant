...
class BaseAgent(ABC):
            ...
            try:
                ...
                if config_manager:
                    ...
                    default_config.update({
                        'max_concurrent_documents': processing_config.get('max_concurrent_documents', Constants.Performance.MAX_CONCURRENT_DOCUMENTS),
                        'batch_size': processing_config.get('batch_size', Constants.Performance.DEFAULT_BATCH_SIZE),
                        'enable_auto_tagging': processing_config.get('enable_auto_tagging', True),
                        'auto_tag_confidence_threshold': processing_config.get('auto_tag_confidence_threshold', Constants.Document.AUTO_TAG_CONFIDENCE_THRESHOLD)
                    })
                    
                    logger.info(f"Loaded configuration from ConfigurationManager for {self.name}")
    ...
    async def process(self, data: Any, priority: TaskPriority = TaskPriority.NORMAL, 
                     metadata: Dict[str, Any] = None, timeout: float = None) -> AgentResult:
        """
        ...
        try:
            ...
            error_msg = f"Task {task_id} timed out after {timeout or self.config['timeout']}s"
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    logger.warning(f"Agent {self.name} attempt {attempt + 1} failed: {e}. Retrying in {retry_delay}s...")
                    await asyncio.sleep(retry_delay * (attempt + 1))  # Exponential backoff
                else:
                    logger.error(f"Agent {self.name} failed after {max_retries + 1} attempts")
                    raise AgentError(f"All retry attempts failed. Last error: {e}", self.name)
    ...
    async def add_task(self, data: Any, priority: TaskPriority = TaskPriority.NORMAL,
    ...
    def _validate_input(self, data: Any, required_fields: List[str] = None) -> None:
        ...
        if required_fields and isinstance(data, dict):
            ...
            if missing_fields:
                raise AgentError(f"Missing required fields: {missing_fields}", self.name)
