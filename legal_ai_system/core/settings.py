    ...
    try:
        ...
        class BaseSettings: # type: ignore[no-redef]
            ...
            def __getattr__(self, name: str) -> Any:
                ...
                if name in self.__class__._default_fields:
                    return self.__class__._default_fields[name]
...
class LegalAISettings(BaseSettings):
    ...
    base_dir: Path = Field(default=Path(__file__).resolve().parent.parent, env="BASE_DIR")
    ...
    def __init__(self, **data: Any):
        ...
        try:
            ...
            if self.auto_tag_confidence_threshold is None:
                self.auto_tag_confidence_threshold = Constants.Document.AUTO_TAG_CONFIDENCE_THRESHOLD
    ...
    # =================== VECTOR STORAGE ===================
    vector_store_type: str = Field(default="hybrid", env="VECTOR_STORE_TYPE")  # faiss, lance, hybrid
    embedding_model: str = Field(default="all-MiniLM-L6-v2", env="EMBEDDING_MODEL")
    embedding_dim: int = Field(default=384, env="EMBEDDING_DIM")  # Will be set in __init__
    ...
    neo4j_password: str = Field(default="CaseDBMS", env="NEO4J_PASSWORD") # Default, should be in .env
    ...
    auto_tag_confidence_threshold: Optional[float] = Field(default=None, env="AUTO_TAG_THRESHOLD")
    tag_history_path: Optional[Path] = Field(default=None, env="TAG_HISTORY_PATH") # Path updated in __init__
    
    # Learning System
    feedback_learning_enabled: bool = Field(default=True, env="FEEDBACK_LEARNING")
    min_feedback_samples: Optional[int] = Field(default=None, env="MIN_FEEDBACK_SAMPLES")
    
    # =================== FILE WATCHING ===================
    enable_file_watching: bool = Field(default=True, env="ENABLE_FILE_WATCHING")
    watch_directories: List[str] = Field(default=[
    ...
    auto_summarize_threshold: Optional[int] = Field(default=None, env="AUTO_SUMMARIZE_THRESHOLD")
    ...
    jurisdiction_hierarchy: List[str] = Field(default=[
        "Federal", "State", "County", "Municipal"
    ], env="JURISDICTION_HIERARCHY")
    
    # Citation Processing
    citation_formats: List[str] = Field(default=[
        "bluebook", "alwd", "mla", "apa"
    ], env="CITATION_FORMATS")
    
    # Violation Detection
    violation_confidence_threshold: Optional[float] = Field(default=None, env="VIOLATION_THRESHOLD")
    ...
    # =================== PERFORMANCE ===================
    # Processing
    max_concurrent_documents: Optional[int] = Field(default=None, env="MAX_CONCURRENT_DOCS")
    ...
    # =================== SECURITY ===================
    # API Security
    rate_limit_per_minute: Optional[int] = Field(default=None, env="RATE_LIMIT_PER_MINUTE")
