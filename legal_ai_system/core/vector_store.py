...
vs_embedding_logger = get_detailed_logger("VectorStore.Embedding", LogCategory.VECTOR_STORE)
...
class VectorStoreState(Enum):
    ...
    ERROR = "error"; MAINTENANCE = "maintenance"; SHUTDOWN = "shutdown"
...
class VectorMetadata:
    ...
    def from_row_dict(cls, row_dict: Dict[str, Any]) -> 'VectorMetadata':
        return cls(
            ...
            tags=json.loads(row_dict['tags']) if isinstance(row_dict.get('tags'), str) else (row_dict.get('tags',[]) or []),
            confidence_score=float(row_dict.get('confidence_score',1.0)),
            embedding_model=str(row_dict.get('embedding_model','unknown')),
            custom_metadata=json.loads(row_dict['custom_metadata']) if isinstance(row_dict.get('custom_metadata'), str) else (row_dict.get('custom_metadata',{}) or {})
        )

@dataclass
class SearchResult:
    vector_id: str; document_id: str; content_preview: str
    similarity_score: float; distance: float
    metadata: VectorMetadata; search_time_sec: float
    index_type_used: str; rank: int
...
class IndexStatistics:
    total_vectors: int = 0; total_adds_session: int = 0; avg_add_time_sec_session: float = 0.0
    total_searches_session: int = 0; avg_search_time_sec_session: float = 0.0
    index_disk_size_mb: float = 0.0; metadata_db_size_mb: float = 0.0 # For on-disk sizes
    cache_hit_rate_session: float = 0.0; cache_hits_session: int = 0; cache_misses_session: int = 0
    last_optimization_at_iso: Optional[str] = None; optimization_runs_count: int = 0
    faiss_index_memory_mb: float = 0.0 # Approx FAISS RAM usage

class EmbeddingProviderVS(ABC):
    def __init__(self, model_name: str, service_config: Optional[Dict[str,Any]] = None):
    ...
    @abstractmethod
    async def embed_texts(self, texts: List[str], batch_size: Optional[int] = None) -> List[List[float]]: pass


class SentenceTransformerEmbeddingProvider(EmbeddingProviderVS):
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2", service_config: Optional[Dict[str,Any]] = None):
    ...
    async def initialize(self):
        if self._initialized_flag:
            self.logger.debug(f"SentenceTransformer '{self.model_name}' already initialized.")
            return
        ...
        try:
            ...
            if not self.model: raise VectorStoreError(f"Failed to load SentenceTransformer model {self.model_name}")
            self.dimension = self.model.get_sentence_embedding_dimension() # type: ignore
            self._initialized_flag = True
            self.logger.info(f"SentenceTransformer '{self.model_name}' loaded successfully.", parameters={'dimension': self.dimension})
        except ImportError:
            self.logger.critical("SentenceTransformers library not found. Install with: pip install sentence-transformers")
            raise ConfigurationError("SentenceTransformers library not installed.", config_key="embedding_provider_dependency")
        except Exception as e:
            self.logger.critical(f"Failed to load SentenceTransformer model '{self.model_name}'.", exception=e, exc_info=True)
            raise VectorStoreError(f"Model loading failed for {self.model_name}", cause=e)
    ...
    async def embed_texts(self, texts: List[str], batch_size: Optional[int] = None) -> List[List[float]]:
        if not self._initialized_flag or not self.model:
            self.logger.warning(f"SentenceTransformer '{self.model_name}' not initialized. Attempting to initialize now.")
            await self.initialize()
            if not self._initialized_flag or not self.model:
                raise VectorStoreError(f"SentenceTransformer model '{self.model_name}' could not be initialized for embedding.")
        ...
        try:
            embeddings_np_arrays: np.ndarray = await loop.run_in_executor(
                ...
                effective_batch_size, False, None, # batch_size, show_progress_bar, output_value
                True, False, None, True # convert_to_numpy, normalize_embeddings (False, manual), convert_to_tensor, device
            )
        except Exception as e:
            self.logger.error(f"Error during SentenceTransformer encode for model '{self.model_name}'.", exception=e, exc_info=True)
            raise VectorStoreError(f"Embedding generation failed with model {self.model_name}", cause=e)
        ...
        vs_perf_logger.performance_metric("EmbedTexts_ST", duration, {'num_texts': len(texts), 'batch_size': effective_batch_size, 'model': self.model_name})
        self.logger.debug(f"Generated {len(embeddings_normalized_lists)} normalized embeddings in {duration}s.")
        return embeddings_normalized_lists


class VectorStore:
    GPU_SUPPORTED_INDEX_TYPES = [IndexType.FLAT, IndexType.IVF, IndexType.HNSW, IndexType.IVFPQ, IndexType.PQ] # PQ can benefit too
    ...
    def __init__(
        ...
        self.ivf_training_threshold = int(self.config.get("ivf_training_threshold", 1024))
        self.ivf_nlist_factor = int(self.config.get("ivf_nlist_factor", 8))
        self.document_index_requires_training = self.index_type in [IndexType.IVF, IndexType.IVFPQ, IndexType.PQ]
        self.document_index_trained = not self.document_index_requires_training
        self.pending_vectors_document: List[np.ndarray] = []
        
        self.entity_index_requires_training = self.index_type in [IndexType.IVF, IndexType.IVFPQ, IndexType.PQ]
        self.entity_index_trained = not self.entity_index_requires_training
        self.pending_vectors_entity: List[np.ndarray] = []
        
        self.metadata_db_path = self.storage_path / "vector_metadata.sqlite" # Changed extension
        self.metadata_mem_cache: Dict[str, VectorMetadata] = {}
        
        self.stats = IndexStatistics()
        self.search_history_log: Deque[Dict[str,Any]] = deque(maxlen=self.config.get("search_history_max_len", 200))
        
        self._async_lock = asyncio.Lock()
        self._sync_lock = threading.RLock() # For purely synchronous, thread-sensitive internal ops
        
        self._optimization_task_internal: Optional[asyncio.Task] = None
        self._stop_background_tasks_event = asyncio.Event() # One event for all background tasks
        self.optimization_request_queue: asyncio.Queue[Dict[str, Any]] = asyncio.Queue(maxsize=10) # Max pending optimization jobs

        self._periodic_save_interval_sec = int(self.config.get("periodic_save_interval_sec", 300))
        self._periodic_save_task_internal: Optional[asyncio.Task] = None

        vector_store_logger.info("=== VectorStore: Instance Creation END (Uninitialized) ===")

    @detailed_log_function(LogCategory.VECTOR_STORE)
    async def initialize(self):
        if self.state not in [VectorStoreState.UNINITIALIZED, VectorStoreState.ERROR]:
            vector_store_logger.warning(f"VectorStore already initialized or in active state: {self.state.value}")
            return
        ...
        try:
            ...
            vector_store_logger.info("VectorStore initialization complete and ready.", parameters=self._get_current_stats_summary())
        except Exception as e:
            self.state = VectorStoreState.ERROR
            vector_store_logger.critical("VectorStore initialization failed critically.", exception=e, exc_info=True)
    ...
    def _initialize_faiss_indexes_sync(self):
        vs_index_logger.info(f"Initializing FAISS indexes (sync).", parameters={'type': self.index_type.value, 'dim': self.dimension})
        ...
        def create_index(index_type_enum: IndexType, dim: int, purpose: str) -> faiss.Index:
            ...
            if index_type_enum == IndexType.HNSW:
                ...
                index.hnsw.efConstruction = int(self.config.get(f"hnsw_efconstruction_{purpose}", 40))
                return index
            ...
            possible_ms = [d for d in [1, 2, 4, 8, 16, 32, 64, dim // 64, dim // 32, dim // 16, dim // 8, dim // 4, dim // 2] if d > 0 and dim % d == 0]
            ...
            if index_type_enum == IndexType.IVFPQ:
                return faiss.IndexIVFPQ(quantizer, dim, nlist, m_pq, nbits_pq, faiss.METRIC_L2)
            
            raise ConfigurationError(f"Unsupported FAISS index type for creation: {index_type_enum.value}", config_key="index_type")
        ...
        try:
            with self._sync_lock: # Protect index creation
                ...
                vs_index_logger.info("FAISS indexes created in memory.", parameters={'doc_idx': type(self.document_index).__name__, 'ent_idx': type(self.entity_index).__name__})
        except Exception as e:
            vs_index_logger.critical("Failed to initialize FAISS indexes.", exception=e, exc_info=True)
            raise VectorStoreError("FAISS index initialization critical failure.", cause=e)
    ...
    async def _load_all_existing_data_async(self):
        ...
        if self.document_index_requires_training: self.document_index_trained = self.document_index.is_trained if self.document_index else False
        if self.entity_index_requires_training: self.entity_index_trained = self.entity_index.is_trained if self.entity_index else False
        
        vector_store_logger.info("Existing data loading process complete.", parameters=self._get_current_stats_summary())
    ...
    def _load_metadata_mem_cache_sync(self):
        ...
        try:
            with self._sync_lock, sqlite3.connect(self.metadata_db_path, timeout=5) as conn:
                ...
                cursor = conn.execute(f"SELECT * FROM vector_metadata ORDER BY last_accessed_iso DESC LIMIT {max_cache_size}")
                
                new_cache: Dict[str, VectorMetadata] = {}
                for row_data in map(dict, cursor.fetchall()):
                    try: new_cache[row_data['vector_id']] = VectorMetadata.from_row_dict(row_data)
                    except (KeyError, TypeError, json.JSONDecodeError, ValueError) as e:
                        vs_cache_logger.warning("Skipping malformed metadata record during cache load.", parameters={'vector_id': row_data.get('vector_id'), 'error': str(e)})
                self.metadata_mem_cache = new_cache # Replace entire cache
            vs_cache_logger.info("Metadata memory cache loaded.", parameters={'cached_items': len(self.metadata_mem_cache)})
        except sqlite3.Error as e:
            vs_cache_logger.error("SQLite error loading metadata cache.", exception=e)
        except Exception as e:
            vs_cache_logger.error("Unexpected error loading metadata cache.", exception=e, exc_info=True)
    ...
    def _start_background_optimization_task(self):
        ...
        self._optimization_task_internal = asyncio.create_task(self._optimization_worker_async())
    ...
    async def _optimization_worker_async(self):
        ...
        while not self._stop_background_tasks_event.is_set():
            try:
                task_data = await asyncio.wait_for(self.optimization_request_queue.get(), timeout=5.0)
                ...
                async with self._async_lock: # Ensure optimization logic has exclusive access
                    if task_type == "OPTIMIZE_INDEX_PERFORMANCE":
                        ...
                        vs_index_logger.warning(f"Unknown optimization task type received: {task_type}")
                self.optimization_request_queue.task_done()
                ...
                vs_index_logger.error("Error in async optimization worker.", exception=e, exc_info=True)
    ...
    def _start_periodic_save_task(self):
        if self._periodic_save_task_internal and not self._periodic_save_task_internal.done():
            vs_index_logger.warning("Periodic save task already running.")
            return
        vs_index_logger.info(f"Starting periodic save task (interval: {self._periodic_save_interval_sec}s).")
        self._periodic_save_task_internal = asyncio.create_task(self._periodic_save_worker_async())
    ...
    async def _periodic_save_worker_async(self):
        ...
        while not self._stop_background_tasks_event.is_set():
            try:
                ...
                if self._stop_background_tasks_event.is_set(): break

                if self.state == VectorStoreState.READY:
                    vs_index_logger.info("Performing periodic save of FAISS indexes.")
                    await self.optimization_request_queue.put({"type": "SAVE_INDEXES_NOW"}) # Use queue for coordinated save
                else:
                    vs_index_logger.debug(f"Skipping periodic save, store state is {self.state.value}.")
            except asyncio.CancelledError:
                vs_index_logger.info("Periodic save task was cancelled.")
                break
            except Exception as e:
                vs_index_logger.error("Error in periodic save worker.", exception=e, exc_info=True)
                await asyncio.sleep(self._periodic_save_interval_sec // 2) # Shorter wait after error
        ...
        with self._sync_lock: # Thread lock for FAISS C++ object access
            ...
            if self.document_index and self.document_index.ntotal > 0:
                ...
                try:
                    ...
                    vs_index_logger.error("Failed to save document FAISS index.", exception=e, exc_info=True)
            ...
            if self.entity_index and self.entity_index.ntotal > 0:
                ...
                try:
                    ...
                    vs_index_logger.error("Failed to save entity FAISS index.", exception=e, exc_info=True)
                    if tmp_ent_idx_path.exists(): os.remove(tmp_ent_idx_path)
    ...
    def _store_metadata_sync(self, metadata: VectorMetadata):
        ...
        try:
            ...
            vs_index_logger.error(f"SQLite error storing metadata for vector_id {metadata.vector_id}.", exception=e, exc_info=True)
            raise DatabaseError(f"Failed to store metadata for {metadata.vector_id}", database_type="sqlite", cause=e)
        except Exception as e:
            vs_index_logger.error(f"Unexpected error storing metadata for {metadata.vector_id}.", exception=e, exc_info=True)
            raise VectorStoreError(f"Unexpected error storing metadata for {metadata.vector_id}", cause=e)

    async def _find_by_content_hash_sync_via_executor(self, content_hash: str) -> Optional[str]:
        """Checks if a content hash already exists, returns its vector_id if so."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self._find_by_content_hash_sync, content_hash)

    def _find_by_content_hash_sync(self, content_hash: str) -> Optional[str]:
        try:
            with self._sync_lock, sqlite3.connect(self.metadata_db_path, timeout=5) as conn:
                cursor = conn.execute("SELECT vector_id FROM vector_metadata WHERE content_hash = ? LIMIT 1", (content_hash,))
                row = cursor.fetchone()
                return row[0] if row else None
        except sqlite3.Error as e:
            vs_cache_logger.error(f"SQLite error finding by content hash {content_hash}.", exception=e)
    ...
    async def add_vector_async(
        ...
        vector_metadata_obj: Optional[VectorMetadata] = None, # Allow passing a pre-filled object
        **kwargs # For other metadata fields like source_file, tags, custom_metadata
    ) -> str:
        if self.state != VectorStoreState.READY:
            raise VectorStoreError(f"VectorStore not ready for adding vectors. Current state: {self.state.value}")
        if not self.dimension: # Should have been set by initialize
            raise VectorStoreError("Vector dimension not set. Store may not be initialized.")

        vector_store_logger.info(f"Request to add vector.", parameters={'doc_id': document_id_ref, 'index': index_target})
        start_time_add = time.perf_counter()
        
        async with self._async_lock: # Ensure atomicity of add + metadata store
            content_hash = hashlib.sha256(content_to_embed.encode('utf-8','ignore')).hexdigest()
            
            # Optional: Deduplication based on content hash
            if self.config.get("deduplicate_on_add", False):
                existing_vector_id = await self._find_by_content_hash_sync_via_executor(content_hash)
                if existing_vector_id:
                    vector_store_logger.info(f"Duplicate content hash found. Returning existing vector_id '{existing_vector_id}'.", 
                                            parameters={'hash': content_hash, 'doc_id': document_id_ref})
                    await self._update_access_stats_async(existing_vector_id) # Update access for existing
            ...
            try:
                ...
                if not embeddings_list or not embeddings_list[0]:
                    raise VectorStoreError("EmbeddingProvider returned no embedding for content.")
                embedding_np = np.array(embeddings_list[0], dtype='float32').reshape(1, -1) # FAISS expects (1, D)
            except Exception as embed_e: # Catch errors from embedding provider
                raise VectorStoreError("Embedding generation failed.", cause=embed_e, details={'doc_id': document_id_ref}) from embed_e
            ...
            target_faiss_index = self.document_index if is_doc_index else self.entity_index
            requires_training = self.document_index_requires_training if is_doc_index else self.entity_index_requires_training
            is_trained_attr = "document_index_trained" if is_doc_index else "entity_index_trained"
            pending_vectors_list = self.pending_vectors_document if is_doc_index else self.pending_vectors_entity
            ...
            vector_id = vector_id_override or self._generate_unique_vector_id(index_target, document_id_ref)
            ...
            self.stats.avg_add_time_sec_session = \
                (self.stats.avg_add_time_sec_session * (self.stats.total_adds_session -1) + duration_add) / self.stats.total_adds_session \
                if self.stats.total_adds_session > 0 else duration_add

            vector_store_logger.info("Vector added successfully.", parameters={
                'vector_id': vector_id, 'doc_id': document_id_ref, 'index': index_target, 'duration_sec': duration_add
            })
            vs_perf_logger.performance_metric(f"AddVector_{index_target}", duration_add, {'content_len': len(content_to_embed)})
            return vector_id
    ...
    async def search_similar_async(
        ...
        top_k: int = 10,
        filter_metadata: Optional[Dict[str, Any]] = None, # For pre-filtering or post-filtering
        min_similarity_score: Optional[float] = None
    ) -> List[SearchResult]:
        if self.state != VectorStoreState.READY:
            vs_search_logger.warning(f"VectorStore not ready for search. State: {self.state.value}")
            return []
        if not self.dimension:
             raise VectorStoreError("Vector dimension not set. Store may not be initialized properly.")

        async with self._async_lock: # Lock during search to prevent index modification issues (if applicable)
            vs_search_logger.info(f"Performing similarity search.", parameters={'query_preview': query_text[:50]+"...", 'index': index_target, 'k': top_k})
            ...
            is_trained_attr = "document_index_trained" if index_target.lower() == "document" else "entity_index_trained"

            if not target_faiss_index:
                raise VectorStoreError(f"Target FAISS index '{index_target}' not initialized for search.")
            
            if getattr(self, ("document_index_requires_training" if index_target.lower()=="document" else "entity_index_requires_training")) \
               and not getattr(self, is_trained_attr):
                vs_search_logger.warning(f"{index_target} index requires training but is not trained. Search may be suboptimal or fail.")
            ...
            # 1. Embed query
            try:
                query_embeddings_list = await self.embedding_provider.embed_texts([query_text])
                if not query_embeddings_list or not query_embeddings_list[0]:
                    raise VectorStoreError("Failed to generate embedding for search query.")
                query_embedding_np = np.array(query_embeddings_list[0], dtype='float32').reshape(1, -1)
            except Exception as embed_e:
                raise VectorStoreError("Query embedding failed.", cause=embed_e) from embed_e
            ...
            if isinstance(target_faiss_index, faiss.IndexHNSW):
                target_faiss_index.hnsw.efSearch = int(self.config.get("hnsw_efsearch", 64)) # Set efSearch for HNSW quality
            elif isinstance(target_faiss_index, faiss.IndexIVF): # IVF based indexes
                target_faiss_index.nprobe = int(self.config.get("ivf_nprobe", 10)) # Set nprobe for IVF quality

            try:
                distances_batch, indices_batch = await loop.run_in_executor(None, target_faiss_index.search, query_embedding_np, top_k)
            except RuntimeError as faiss_e: # Catch FAISS specific runtime errors
                raise VectorStoreError("FAISS search operation failed.", cause=faiss_e) from faiss_e

            search_duration_faiss = time.perf_counter() - start_time_search # Only FAISS search time
            
            # 3. Retrieve metadata and construct SearchResult objects
            results: List[SearchResult] = []
            cache_hits = 0
            # FAISS returns internal integer IDs. We need a way to map these back to our application vector_ids.
            # This requires IndexIDMap if we use string IDs with FAISS, or a separate mapping layer.
            # For simplicity, if we don't use IndexIDMap, we assume our vector_id for metadata somehow relates to
            # the order of addition or we search metadata by other means (e.g. if we stored vectors with our custom IDs using add_with_ids)
            # Let's assume we need to fetch metadata for each FAISS index `idx`.
            # This is where the mapping from FAISS sequential ID to our application's VectorMetadata.vector_id is crucial.
            # If we add vectors sequentially without `add_with_ids`, `indices_batch[0]` contains the internal FAISS IDs (0 to ntotal-1).
            # We need a reverse map: faiss_internal_id -> application_vector_id.
            # This is complex to maintain with additions/deletions if not using FAISS's IDMap.
            # For now, conceptual: assume we can get our vector_id from FAISS internal id `idx`.
            # This placeholder needs a robust solution in a production system.
            # One way: When adding, store mapping: faiss_idx -> our_vector_id.
            # OR query metadata DB by some other property if possible (less efficient)

            # Placeholder: For now, construct a dummy vector_id for demonstration if direct mapping is not set up.
            # In a real system with IndexFlatL2/HNSWFlat, indices_batch[0] ARE the 0-based indices.
            # We need a separate list or DB table `faiss_id_to_app_vector_id_map`.

            # For this example, let's assume our `VectorMetadata.vector_id` was constructed as `f"{index_target}_FAISSINTERNALID_{content_hash_short}"`
            # OR, better, when we add to FAISS, we use `add_with_ids` if we have our own int64 IDs.
            # If we don't use `add_with_ids`, then `indices_batch[0]` are the sequential 0-based IDs.
            # We need to retrieve metadata using these. A common pattern is to store metadata with these sequential FAISS IDs if not using IDMap.

            # Simpler: iterate through FAISS indices and fetch associated metadata.
            # Let's assume we have a method to get app vector_id from FAISS idx (e.g., a list of all app vector_ids in FAISS order).
            # This is a major simplification and needs careful design.
            # A better design: VectorMetadata table has a faiss_internal_id column.

            faiss_indices = indices_batch[0]
            faiss_distances = distances_batch[0]

            # Fetch all potentially relevant metadata in one go (if possible and efficient)
            # For now, fetch one by one.
            for rank_idx, internal_faiss_id in enumerate(faiss_indices):
                if internal_faiss_id == -1: continue # No more results or invalid index
                
                # This is the CRITICAL part: mapping internal_faiss_id to your application's VectorMetadata.vector_id
                # Option 1: If you have a direct list map (fragile with deletes unless index is rebuilt):
                #    app_vector_id = self.faiss_id_to_app_id_list[internal_faiss_id]
                # Option 2: Query metadata DB for a vector whose `faiss_internal_id` (a new column you'd add) matches.
                # Option 3 (Simplistic, assumes vector_id contains faiss_id - NOT ROBUST):
                #    vector_id_to_find = f"{index_target}_SOMEPREFIX_{internal_faiss_id}"
                
                # For this example, we'll assume _get_metadata_by_faiss_id_async exists.
                # This helper would query your metadata store using the FAISS internal ID.
                metadata_obj = await self._get_metadata_by_faiss_internal_id_async(internal_faiss_id, index_target)

                if metadata_obj:
                    if metadata_obj.vector_id in self.metadata_mem_cache: cache_hits +=1
                    ...
                    similarity = 1.0 - (faiss_distances[rank_idx] / 2.0) if metadata_obj.vector_norm else 0.0 # Assuming normalized vectors, L2 distance is squared
                    ...
                    results.append(SearchResult(
                        vector_id=metadata_obj.vector_id, document_id=metadata_obj.document_id,
                        content_preview=metadata_obj.content_preview, similarity_score=round(similarity, 4),
                        distance=round(float(faiss_distances[rank_idx]), 4), metadata=metadata_obj,
                        search_time_sec=round(search_duration_faiss, 5), # Time for this specific result is part of batch time
                        index_type_used=self.index_type.value, rank=rank_idx + 1
                    ))
                    await self._update_access_stats_async(metadata_obj.vector_id, metadata_obj.last_accessed_iso)
                else:
                    vs_search_logger.warning(f"Metadata not found for FAISS internal_id {internal_faiss_id} in {index_target} index.")

            # Apply post-filtering if any
            if filter_metadata:
                results = self._apply_filters_to_results(results, filter_metadata)

            total_search_pipeline_duration = round(time.perf_counter() - start_time_search, 3)
            self.stats.total_searches_session += 1
            self.stats.avg_search_time_sec_session = \
                (self.stats.avg_search_time_sec_session * (self.stats.total_searches_session -1) + total_search_pipeline_duration) / self.stats.total_searches_session \
                if self.stats.total_searches_session > 0 else total_search_pipeline_duration
            
            if top_k > 0: self.stats.cache_hit_rate_session = (self.stats.cache_hit_rate_session * (self.stats.total_searches_session -1) + (cache_hits/top_k)) / self.stats.total_searches_session if self.stats.total_searches_session > 0 else (cache_hits/top_k)
            self.stats.cache_hits_session += cache_hits
            self.stats.cache_misses_session += (top_k - cache_hits) # Approximate misses

            self.search_history_log.append({'query': query_text[:100], 'num_results': len(results), 'timestamp_iso': datetime.now(timezone.utc).isoformat()})
    ...
    async def _get_metadata_async_from_db_or_cache(self, vector_id: str) -> Optional[VectorMetadata]:
        ...
        if vector_id in self.metadata_mem_cache:
            ...
            self.metadata_mem_cache[vector_id].last_accessed_iso = datetime.now(timezone.utc).isoformat()
        ...
        vs_cache_logger.debug(f"Metadata cache miss for vector_id '{vector_id}'. Fetching from DB.")
        loop = asyncio.get_event_loop()
        row_dict = await loop.run_in_executor(None, self._get_metadata_from_db_sync, vector_id)
    ...
    def _get_metadata_from_db_sync(self, vector_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self._sync_lock, sqlite3.connect(self.metadata_db_path, timeout=5) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute("SELECT * FROM vector_metadata WHERE vector_id = ?", (vector_id,))
                row = cursor.fetchone()
                return dict(row) if row else None
        except sqlite3.Error as e:
            vs_cache_logger.error(f"SQLite error fetching metadata for '{vector_id}'.", exception=e)
            return None

    def _apply_filters_to_results(self, results: List[SearchResult], filters: Dict[str, Any]) -> List[SearchResult]:
        """Applies post-search filters to results based on metadata criteria."""
        if not filters: return results
        vs_search_logger.debug(f"Applying {len(filters)} post-search filters to {len(results)} results.")
        ...
        for res in results:
            ...
            for key, expected_value in filters.items():
                ...
                if isinstance(expected_value, list): # If filter value is a list, check if actual_value is in it
                    if actual_value not in expected_value:
                        match_all_filters = False; break
        ...
        vs_search_logger.debug(f"Filtering reduced results from {len(results)} to {len(filtered_results)}.")
        return filtered_results

    async def _update_access_stats_async(self, vector_id: str, current_last_accessed_iso: str):
        ...
        async def update_db():
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self._update_access_stats_sync, vector_id, new_last_accessed_iso)
    ...
    def _update_access_stats_sync(self, vector_id: str, new_last_accessed_iso: str):
        try:
            ...
            vs_index_logger.warning(f"Failed to update access stats for vector '{vector_id}'.", exception=e)

    @detailed_log_function(LogCategory.VECTOR_STORE_DB)
    async def update_vector_metadata_async(self, vector_id: str, metadata_updates: Dict[str, Any]):
        """Updates specific fields in a vector's metadata."""
        if not vector_id or not metadata_updates:
            vector_store_logger.warning("Update_vector_metadata_async called with empty vector_id or updates.")
            return
        
        async with self._async_lock: # Ensure atomicity with cache and DB
            vs_cache_logger.debug(f"Updating metadata for vector_id '{vector_id}'.", parameters=metadata_updates)
            
            # Update in-memory cache first
            if vector_id in self.metadata_mem_cache:
                cached_meta = self.metadata_mem_cache[vector_id]
                for key, value in metadata_updates.items():
                    if hasattr(cached_meta, key): setattr(cached_meta, key, value)
                    else: cached_meta.custom_metadata[key] = value # Put in custom_metadata if not a direct field
                cached_meta.last_accessed_iso = datetime.now(timezone.utc).isoformat() # Also treat as access
                cached_meta.access_count +=1
            
            # Persist to DB
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self._update_metadata_in_db_sync, vector_id, metadata_updates)
    ...
    def _update_metadata_in_db_sync(self, vector_id: str, updates: Dict[str, Any]):
        ...
        set_clauses = []
        params = []
        direct_fields = [f.name for f in fields(VectorMetadata)] # Get field names from dataclass

        custom_meta_updates = updates.pop('custom_metadata', {}) # Handle custom_metadata separately if passed in root

        for key, value in updates.items():
            if key in direct_fields: # Is it a direct field of VectorMetadata?
                # Handle specific types for DB storage
                if isinstance(value, datetime): value_to_store = value.isoformat()
                elif isinstance(value, (list, dict)) and key in ['tags', 'custom_metadata']: value_to_store = json.dumps(value)
        ...
        if not set_clauses:
            if custom_meta_updates: # Only custom_metadata to update
                ...
                vs_index_logger.warning(f"Updating only custom_metadata for {vector_id} requires specific JSON handling not fully implemented here.")
            else: return
        ...
        try:
            ...
            vs_index_logger.error(f"SQLite error updating metadata for '{vector_id}'.", exception=e)
            raise DatabaseError(f"Failed to update metadata for {vector_id}", database_type="sqlite", cause=e)
    ...
    async def delete_vector_async(self, vector_id: str, index_target: str = "document"):
        ...
        async with self._async_lock:
            ...
            vector_store_logger.info(f"Vector '{vector_id}' metadata deleted. FAISS index status needs monitoring for actual removal.")

    def _delete_metadata_sync(self, vector_id: str):
        try:
            with self._sync_lock, sqlite3.connect(self.metadata_db_path, timeout=5) as conn:
                conn.execute("DELETE FROM vector_metadata WHERE vector_id = ?", (vector_id,))
                conn.commit()
        except sqlite3.Error as e:
            vs_index_logger.error(f"SQLite error deleting metadata for '{vector_id}'.", exception=e)
            raise DatabaseError(f"Failed to delete metadata for {vector_id}", database_type="sqlite", cause=e)
    ...
    def _update_disk_usage_stats(self):
        ...
        try:
            ...
            self.stats.index_disk_size_mb = round((doc_idx_size + ent_idx_size) / (1024*1024), 2)
    ...
    async def close(self):
        ...
        if self._periodic_save_task_internal:
            ...
            try: await self._periodic_save_task_internal
            except asyncio.CancelledError: vector_store_logger.debug("Periodic save task cancelled.")
            except Exception as e: vector_store_logger.error("Error stopping periodic save task.", exception=e)
            self._periodic_save_task_internal = None

        # Stop optimization worker task
        if self._optimization_task_internal:
            try: await self.optimization_request_queue.put(None) # Send sentinel
            except Exception as q_e: vector_store_logger.warning(f"Error putting sentinel on optimization queue: {q_e}") # Queue might be full or closed
            
            # Give it a moment to process sentinel
            try: await asyncio.wait_for(self._optimization_task_internal, timeout=7.0)
            except asyncio.TimeoutError: 
                vector_store_logger.warning("Optimization worker task did not finish in time. Cancelling.")
                self._optimization_task_internal.cancel()
                try: await self._optimization_task_internal
                except asyncio.CancelledError: vector_store_logger.debug("Optimization worker task cancelled.")
            except Exception as e: vector_store_logger.error("Error stopping optimization worker task.", exception=e)
            self._optimization_task_internal = None

        # Perform final save of FAISS indexes
        vector_store_logger.info("Performing final save of FAISS indexes before shutdown.")
        await self._save_faiss_indexes_async_wrapper()
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "embedding_provider": {
                ...
                "initialized": self.embedding_provider._initialized_flag if hasattr(self.embedding_provider, '_initialized_flag') else "unknown"
            },
            "index_config": {
                "type": self.index_type.value,
                "gpu_enabled": self.enable_gpu,
                "doc_index_vectors": self.document_index.ntotal if self.document_index else 0,
                "doc_index_trained": self.document_index.is_trained if self.document_index and hasattr(self.document_index, 'is_trained') else self.document_index_trained,
                "entity_index_vectors": self.entity_index.ntotal if self.entity_index else 0,
                "entity_index_trained": self.entity_index.is_trained if self.entity_index and hasattr(self.entity_index, 'is_trained') else self.entity_index_trained,
            },
            "statistics": asdict(self.stats), # Convert dataclass to dict
            "background_tasks": {
                "optimization_worker_running": bool(self._optimization_task_internal and not self._optimization_task_internal.done()),
                "periodic_save_running": bool(self._periodic_save_task_internal and not self._periodic_save_task_internal.done()),
            },
            # "test_search_status": test_search_ok
        }

    async def get_service_status(self) -> Dict[str, Any]: # For service container interface
        """Simplified status for service discovery, more detailed in health_check."""
        return {
            "name": "VectorStore",
            "status": "ready" if self.state == VectorStoreState.READY else self.state.value,
            "total_vectors": self.stats.total_vectors,
            "index_type": self.index_type.value,
            "last_updated_iso": datetime.now(timezone.utc).isoformat() # Generic timestamp
        }
...
def create_vector_store(service_container: Any, service_config_override: Optional[Dict[str, Any]] = None) -> VectorStore:
    """
    ...
    """
    cfg_source = service_config_override if service_config_override is not None else service_container
    ...
    try:
        ...
        embedding_provider_instance: EmbeddingProviderVS = service_container.get_service("embedding_provider")
        if not hasattr(embedding_provider_instance, 'dimension') or embedding_provider_instance.dimension is None:
            ...
            vector_store_logger.warning("Embedding provider from service container does not have dimension. "
            ...
            raise ConfigurationError("EmbeddingProvider from service_container is not initialized (no dimension).")
            
    except Exception as e: # Catch if service "embedding_provider" is not found or other issues
        vector_store_logger.critical(f"Failed to get/initialize EmbeddingProvider from service_container: {e}. VectorStore cannot be created.", exc_info=True)
        raise ConfigurationError("Failed to obtain a valid EmbeddingProvider for VectorStore.", cause=e, config_key="embedding_provider_service")

    vs_instance = VectorStore(
        storage_path_str=str(vs_cfg.get("STORAGE_PATH", "./storage/vector_store_data_default")),
