...
class ErrorContext:
    ...
    )  # Consider what system state is safe/useful to log
...
class LegalAIException(Exception):
    ...
    def __init__(
        ...
        self.context = (
            context or self._capture_context()
        )  # Capture context after other fields are set
        self.cause = cause
        ...
        self.stack_trace = (
            traceback.format_exc() if cause else traceback.format_stack()[-3]
        )  # Get relevant part of stack
    ...
    def _capture_context(self) -> ErrorContext:
        ...
        try:
            ...
            error_logger.warning(
                "psutil not installed, cannot get detailed memory/CPU usage for errors."
            )
    ...
    def _log_exception_creation(self):
        ...
        error_logger.error(
            f"Exception Created: {self.__class__.__name__} - {self.message}",
            parameters=log_params,
            exception=self.cause if self.cause else self,
        )  # Log the cause if available

        # Log forensic information
        forensics_logger.info(
            f"Exception Forensics: {self.exception_id}",
            parameters={
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "context": {
                ...
                "parameters": {
                    k: str(v)[:200] for k, v in self.context.parameters.items()
                },  # Truncate params
            ...
            "stack_trace_summary": self.stack_trace.splitlines()[
                -3:
...
class ConfigurationError(LegalAIException):
    ...
    def __init__(
        ...
        if config_key:
            kwargs.setdefault("technical_details", {}).update(
...
class ProcessingError(LegalAIException):
    """Generic processing error for service operations."""
...
class ServiceLayerError(LegalAIException):
...
class AgentProcessingError(AgentExecutionError):
    ...
    def __init__(
        self, message: str, underlying_exception: Optional[Exception] = None, **kwargs
    ):
        kwargs.setdefault("category", ErrorCategory.AGENT)
        kwargs.setdefault("recovery_strategy", ErrorRecoveryStrategy.RETRY)
        if underlying_exception is not None:
            kwargs.setdefault("technical_details", {})["cause"] = str(
...
class LLMProviderError(LegalAIException):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if api_response:
            technical_details["api_response"] = api_response[
                :500
            ]  # Truncate long API responses
...
class DatabaseError(LegalAIException):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if query:
            technical_details["query_summary"] = query[
                :200
            ]  # Log summary, not full query for security
...
class SecurityError(LegalAIException):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        security_error_logger.critical(
...
class ValidationError(LegalAIException):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        )  # Usually a data issue, not system critical
        kwargs.setdefault("recovery_strategy", ErrorRecoveryStrategy.USER_INTERVENTION)
        ...
        if actual_value is not None:
            technical_details["actual_value_summary"] = str(actual_value)[
                :200
            ]  # Truncate for security/log size
...
class ErrorHandler:
    ...
    def __init__(self):
        ...
        self.error_history: List[LegalAIException] = (
            []
        )  # Consider capping size or periodic cleanup
        ...
        self.error_statistics = {
            ...
            "by_recovery_strategy": {
                strategy.value: 0 for strategy in ErrorRecoveryStrategy
            },  # Corrected key
        }
    ...
    def handle_exception(
        ...
        context_override: Optional[
            ErrorContext
        ] = None,  # Renamed context to context_override
        ...
        """
        error_logger.info(
            f"Handling exception: {type(exception).__name__} - {str(exception)[:100]}..."
        )  # Log snippet
        ...
        if not isinstance(exception, LegalAIException):
            ...
            if (
                context_override
            ):  # If it's already LegalAIException but we want to override context
        ...
        if (
            attempt_recovery
            and legal_exception.recovery_strategy != ErrorRecoveryStrategy.NONE
        ):
    ...
    def _update_error_statistics(self, exception: LegalAIException):
        ...
        self.error_statistics["by_recovery_strategy"][
            exception.recovery_strategy.value
        ] += 1  # Corrected key
    ...
    def _attempt_recovery(self, exception: LegalAIException) -> bool:
        ...
        recovery_logger.info(
            f"Attempting recovery for {exception.exception_id} using {exception.recovery_strategy.value}"
        )
        ...
        try:
            ...
            elif exception.recovery_strategy in [
                ErrorRecoveryStrategy.USER_INTERVENTION,
                ErrorRecoveryStrategy.SYSTEM_RESTART,
            ]:
                recovery_logger.info(
                    f"Recovery strategy {exception.recovery_strategy.value} requires external action for {exception.exception_id}"
                )
                return False  # Not automatically recoverable here
            else:  # ErrorRecoveryStrategy.NONE or unknown
                recovery_logger.info(
                    f"No automatic recovery action for strategy {exception.recovery_strategy.value} of {exception.exception_id}"
                )
                return False

            recovery_logger.info(
                f"Recovery attempt for {exception.exception_id} {'succeeded' if success else 'failed'}"
            )
            return success

        except Exception as recovery_error:
            recovery_logger.error(
                f"Recovery attempt itself failed for {exception.exception_id}",
    ...
    def _retry_operation(self, exception: LegalAIException) -> bool:
        ...
        recovery_logger.trace(
            f"Implementing retry strategy for {exception.exception_id} (placeholder)"
        )
    ...
    def _fallback_operation(self, exception: LegalAIException) -> bool:
        ...
        recovery_logger.trace(
            f"Implementing fallback strategy for {exception.exception_id} (placeholder)"
        )
    ...
    def _graceful_degradation(self, exception: LegalAIException) -> bool:
        ...
        recovery_logger.trace(
            f"Implementing graceful degradation for {exception.exception_id} (placeholder)"
        )
    ...
    def _data_recovery(self, exception: LegalAIException) -> bool:
        ...
        recovery_logger.trace(
            f"Implementing data recovery for {exception.exception_id} (placeholder)"
        )
    ...
    def _notify_user(self, exception: LegalAIException):
        ...
        error_logger.info(
            f"User Notification Triggered for {exception.exception_id}: {user_msg}",
    ...
    def get_error_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detected_patterns": {
                k: v for k, v in self.error_patterns.items() if v >= 3
            },  # Corrected key
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if (
                    category_override != ErrorCategory.SYSTEM
                ...
                handle_error(
                    legal_exception, user_notification=user_notification_override
                )
