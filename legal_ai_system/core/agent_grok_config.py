...
try:
    ...
    try:
        ...
        class LLMProviderEnum:
            XAI = "xai"
        # Mock functions
        def create_grok_config(**kwargs): return LLMConfig(**kwargs)
        def create_grok_legal_settings(**kwargs): return kwargs
        ...
        LEGAL_ANALYSIS_PARAMS = {
            "legal_temperature": 0.3,
            "citation_temperature": 0.1,
...
def create_agent_grok_config(api_key: Optional[str] = None) -> LLMConfig:
    ...
    if not resolved_api_key:
        raise ValueError("XAI API key required. Set XAI_API_KEY environment variable or provide api_key parameter.")
    ...
    return LLMConfig(
        ...
        temperature=LEGAL_ANALYSIS_PARAMS["legal_temperature"],  # Use legal-optimized temperature
...
def configure_agent_service_container(service_container: Any, api_key: Optional[str] = None) -> None:
    """
...
def ensure_agents_use_grok(service_container: Any = None, api_key: Optional[str] = None) -> bool:
    """
