...
class XAIGrokClient:
    ...
    def analyze_document(self, document_text: str, analysis_type: str = "legal_analysis", 
                        model: str = DEFAULT_GROK_MODEL) -> Dict[str, Any]:
        ...
        try:
            ...
            model_config = GROK_MODELS_CONFIG.get(model, GROK_MODELS_CONFIG[DEFAULT_GROK_MODEL])
    ...
    def detect_violations(self, document_text: str, model: str = DEFAULT_GROK_MODEL) -> Dict[str, Any]:
        """Detect legal violations in document"""
        return self.analyze_document(document_text, "violation_detection", model)
    
    def extract_entities(self, document_text: str, model: str = DEFAULT_GROK_MODEL) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.post(
                ...
                json={
                    ...
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": model_config["max_tokens"],
                    "temperature": 0.1  # Low temperature for entity extraction
                },
                timeout=60
            )
    ...
    def summarize_document(self, document_text: str, model: str = DEFAULT_GROK_MODEL) -> Dict[str, Any]:
        """Create document summary"""
        return self.analyze_document(document_text, "document_summary", model)
    
    def _get_analysis_prompt(self, analysis_type: str, model: str, document_text: str) -> str:
        ...
        try:
            ...
            if 'get_optimized_prompt' in globals():
                return get_optimized_prompt(analysis_type, model, document_text=document_text)
        except Exception as e:
...
class XAIIntegratedGUI:
        ...
        if api_key:
            ...
            with col1:
                selected_model = st.selectbox(
                    ...
                    format_func=lambda x: next(m["display_name"] for m in available_models if m["name"] == x),
            ...
            with col2:
                if selected_model:
                    ...
                    st.write(f"• Reasoning: {'Yes' if model_info['reasoning'] else 'No'}")
                    st.write(f"• Context: {model_info['context_length']:,} tokens")
                    st.write(f"• Max Output: {model_info['max_tokens']:,} tokens")
        
        return api_key, selected_model if 'selected_model' in locals() else DEFAULT_GROK_MODEL
    ...
    def render_xai_document_processor():
        ...
        if not api_key:
            st.warning("⚠️ Please configure your XAI API key in the settings above to use Grok models.")
            return
        ...
        if uploaded_file:
            ...
            with col1:
                analysis_types = st.multiselect(
                    "Analysis Types",
                    ["Legal Analysis", "Violation Detection", "Entity Extraction", "Document Summary"],
                ...
                try:
                    ...
                    for i, analysis_type in enumerate(analysis_types):
                        ...
                        if analysis_type == "Legal Analysis":
                            result = client.analyze_document(document_text, "legal_analysis", selected_model)
                        elif analysis_type == "Violation Detection":
                            result = client.detect_violations(document_text, selected_model)
                        elif analysis_type == "Entity Extraction":
                            result = client.extract_entities(document_text, selected_model)
                        elif analysis_type == "Document Summary":
                            result = client.summarize_document(document_text, selected_model)
                    ...
                    for analysis_type, result in results.items():
                        with st.expander(f"📋 {analysis_type}", expanded=True):
                            if result["status"] == "SUCCESS":
                                ...
                                st.write(result.get("analysis") or result.get("entities") or "No content returned")
                                ...
                                if "token_usage" in result and result["token_usage"]:
                                    ...
                                    col2.metric("Tokens Used", usage.get("total_tokens", "N/A"))
                                    col3.metric("Completion Tokens", usage.get("completion_tokens", "N/A"))
                            else:
...
def check_xai_setup() -> Dict[str, Any]:
    ...
    setup_status = {
        "api_key_configured": bool(os.getenv("XAI_API_KEY") or st.session_state.get("xai_api_key")),
