...
class RealTimeAnalysisWorkflow:
    ...
    async def process_document_realtime(
        ...
        async with self.processing_lock:
            try:
                self.logger.info(
                    f"Starting real-time analysis for: {document_path}"
                )

                dp_start = time.time()
                doc_result = await self.document_processor.process(
                    document_path, **kwargs
                )
                processing_times = {
                    "document_processing": time.time() - dp_start
                }

                rw_start = time.time()
                rewrite = await self.document_rewriter.rewrite_text(
                    doc_result.data.content
                )
                processing_times["document_rewriting"] = time.time() - rw_start

                hybrid_start = time.time()
                hybrid_result = await self.hybrid_extractor.extract_from_document(
                    doc_result
                )
                processing_times["hybrid_extraction"] = time.time() - hybrid_start
                ...
                legal_doc = self._create_legal_document(
                    ...
                    document_path,
                    document_id,
                    text_override=rewrite.corrected_text,
                )
                ontology_result = await self.ontology_extractor.process(legal_doc)
                processing_times["ontology_extraction"] = (
                    time.time() - ontology_start
                )
                ...
                if (
                    ...
                    and self.documents_processed % self.auto_optimization_threshold
                    == 0
                ):
