...
@detailed_log_function(
    LogCategory.SYSTEM
)  # Assuming detailed_logging is available when this is called by other parts of the system
def initialize_system(
    is_first_run_setup: bool = False,
) -> Dict[str, Any]:  # Added param
    ...
    try:
            ...
            try:
                if not dir_path_obj.exists():
                    ...
                    initializer_logger.info(
                        f"Directory created.", parameters={"path": str(dir_path_obj)}
                    )
                else:
                    initializer_logger.debug(
                        f"Directory already exists.",
                        parameters={"path": str(dir_path_obj)},
                    )
        ...
        if is_first_run_setup:
            initializer_logger.info(
                "Performing first-run setup: Creating config files..."
            )
    ...
    for db_filename, creation_func in databases_to_init.items():
        ...
        try:
            if not db_file_path.exists():
                initializer_logger.debug(
                    f"Database file '{db_filename}' not found. Creating..."
                )
                creation_func(db_file_path)
                results["created"].append(str(db_file_path))
                initializer_logger.info(
                    f"Database created.", parameters={"db_name": db_filename}
                )
            else:
                initializer_logger.debug(
                    f"Database already exists.", parameters={"db_name": db_filename}
                )
...
def _execute_schema(db_path: Path, schema_sql: str):
    ...
    try:
        ...
        initializer_logger.error(
            f"SQLite error executing schema for {db_path.name}", exception=e
        )
...
def _create_knowledge_graph_db_schema(
    db_path: Path,
):  # For local caching if Neo4j is down or not primary
    schema = """
...
"""
        try:
            ...
            initializer_logger.info(
                f"Created example environment file.",
                parameters={"path": str(env_example_path)},
            )
        except IOError as e:
            initializer_logger.error(
                f"Failed to create .env.example file.", exception=e
            )
...
def test_system_health() -> Dict[str, Any]:
    ...
    if LOGS_DIR_INIT.exists() and LOGS_DIR_INIT.is_dir():
        ...
        health["component_checks"][
            "logging_directory"
        ] = "ERROR: Missing or not a directory"
    ...
    if DATABASES_DIR_INIT.exists() and DATABASES_DIR_INIT.is_dir():
        ...
        if db_files:
            ...
            try:
                ...
                health["component_checks"][
                    "database_connectivity_basic"
                ] = f"ERROR: {str(e)}"
                health["issues_found"].append(
                    f"Basic DB connectivity failed for {db_files[0].name}."
                )
            ...
            ] = "WARNING: No .db files found"
            health["issues_found"].append(
                "No SQLite database files found in databases directory."
            )
            health["overall_status"] = "DEGRADED"
    else:
        health["component_checks"][
...
if __name__ == "__main__":
    ...
    )  # Assume first run if executed directly
