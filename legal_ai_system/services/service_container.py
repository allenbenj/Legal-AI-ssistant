...
try:
    ...
    from legal_ai_system.core.unified_exceptions import ConfigurationError, SystemInitializationError
    from legal_ai_system.core.security_manager import AuthenticationManager
except ImportError:
...
async def create_service_container(app_settings: Optional[Any] = None) -> ServiceContainer: # app_settings can be LegalAISettings
    ...
    if not persistence_manager_service or not persistence_manager_service.initialized: # Check if it got initialized by get_service
        ...
        if hasattr(persistence_manager_service, 'initialize') and not getattr(persistence_manager_service, 'initialized', True):
             service_container_logger.info("Explicitly initializing persistence_manager after fetching.")
             await persistence_manager_service.initialize()

    if not persistence_manager_service or not getattr(persistence_manager_service, 'initialized', False): # Final check before use
