...
class MemoryManager:
    ...
    async def create_session(
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(
                None, self._create_session_sync, session_id, session_name, metadata
            )
    ...
    def _add_context_entry_sync(
        self, session_id: str, entry_type: str, content: str, importance_score: float
    ) -> None:
    ...
    async def get_session_context(
        self, session_id: str, max_entries: int = 50, min_importance: float = 0.0
    ) -> List[Dict[str, Any]]:
    ...
    def _get_session_context_sync(
        self, session_id: str, max_entries: int, min_importance: float
    ) -> List[Dict[str, Any]]:
    ...
    async def get_agent_decisions(
        ...
        try:
            decisions = await asyncio.get_event_loop().run_in_executor(
                None, self._get_agent_decisions_sync, agent_name, session_id, limit
            )
            return decisions
    ...
    def _get_agent_decisions_sync(
        self, agent_name: Optional[str], session_id: Optional[str], limit: int
    ) -> List[Dict[str, Any]]:
    ...
    async def optimize_context_window(
        self, session_id: str, target_tokens: int = 16000
    ) -> None:
        ...
        ----------
        session_id:
            The session to optimize context for.
        target_tokens:
            Rough token budget to retain after pruning. This value is an
            approximation because we do not perform true tokenization here.
        """
        if not self._initialized:
            raise RuntimeError("MemoryManager not initialized")
    ...
    def _optimize_context_window_sync(
        self, session_id: str, target_tokens: int
    ) -> None:
        """Synchronous context window optimization.

        Entries are ordered by importance (descending) and recency. We keep
        entries until the approximate token count exceeds ``target_tokens``. The
        token count is estimated using a simple whitespace split which is
        sufficient for coarse pruning but not exact.
        """
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                """
                SELECT entry_id, content, importance_score, created_at
            ...
            for row in entries:
                entry_tokens = len(row["content"].split())
                if tokens_so_far + entry_tokens <= target_tokens:
                    keep_ids.append(row["entry_id"])
                    tokens_so_far += entry_tokens
                else:
                    # Once the budget is exceeded we stop collecting IDs to keep
                    # and remove the remaining rows in a single query below
                    break

            if len(keep_ids) < len(entries):
                placeholders = ",".join("?" for _ in keep_ids)
                if keep_ids:
                    conn.execute(
                        f"DELETE FROM context_entries WHERE session_id = ? AND entry_id NOT IN ({placeholders})",
                        [session_id] + keep_ids,
                    )
                else:
                    conn.execute(
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "healthy", "statistics": stats, "db_path": self.db_path}
        except Exception as e:
