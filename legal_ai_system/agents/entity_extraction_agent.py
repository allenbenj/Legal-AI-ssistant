...
# Initialize logger
entity_extraction_logger = get_detailed_logger("StreamlinedEntityExtractionAgent", LogCategory.AGENT)
...
class StreamlinedEntityExtractionAgent(BaseAgent):
    ...
    def __init__(self, service_container: Optional[Any] = None, **kwargs):
        ...
        entity_extraction_logger.info("StreamlinedEntityExtractionAgent initialized",
                                     parameters={'threshold': self.confidence_threshold})
    
    @detailed_log_function(LogCategory.AGENT)
    async def extract_entities(self, document: LegalDocument) -> StreamlinedEntityExtractionOutput:
        """Extract entities from a legal document."""
        start_time = datetime.now(timezone.utc)
        
        try:
            entity_extraction_logger.info("Starting entity extraction",
            ...
            entity_extraction_logger.info("Entity extraction completed",
                                         parameters={'document_id': document.id,
            ...
            entity_extraction_logger.error("Entity extraction failed",
                                          parameters={'document_id': document.id},
                                          exception=e)
    ...
    async def process(self, data: Any) -> AgentResult[StreamlinedEntityExtractionOutput]:
        """BaseAgent interface implementation."""
        try:
            if isinstance(data, LegalDocument):
                result = await self.extract_entities(data)
                return AgentResult[StreamlinedEntityExtractionOutput](
                    success=True,
                    data=result,
                    agent_name=self.name
                )
            else:
                raise AgentError("Invalid input data type for StreamlinedEntityExtractionAgent")
