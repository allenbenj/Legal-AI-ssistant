...
class AutoTaggingOutput:
    ...
    extracted_entity_tags: Dict[str, List[Dict[str, Any]]] = field(
        default_factory=dict
    )  # e.g. {"court": [{"name": "Supreme Court", "confidence": 0.8}]}
    generated_subject_tags: List[str] = field(
        default_factory=list
    )  # e.g. ["subject:due_process"]
    all_generated_tags: List[str] = field(
        default_factory=list
    )  # Combined, formatted tags
    suggested_new_tags: List[str] = field(
        default_factory=list
    )  # Tags not in existing_tags metadata
...
class AutoTaggingAgent(BaseAgent, MemoryMixin):
    ...
    def __init__(self, service_container: Any, **config: Any):
        ...
        self.logger: Any = get_detailed_logger(
            self.__class__.__name__, LogCategory.AGENT
        )
        ...
        self.enable_llm_tag_generation = bool(
            config.get("enable_llm_tag_generation", True)
        )
        self.max_text_for_llm = int(
            config.get("max_text_for_llm_autotag", 3000)
        )  # Chars
        ...
        self.unified_memory_manager: Optional[UnifiedMemoryManager] = self._get_service(
        ...
        self.feedback_session_log: List[Dict[str, Any]] = (
            []
        )  # In-memory log for current agent instance
    ...
    async def initialize_agent_async(
        self,
    ):  # Optional: called by a system if agent needs async setup
    ...
    def _init_tagging_frameworks_from_config(self):
        ...
        default_doc_type_patterns = {
            ...
            "order": [
                ...
                r"judgment\s+and\s+order",
                r"minute\s+order",
            ],
            "complaint": [
                r"complaint\s+for\s+\w+",
                r"plaintiff\s+\w+\s+alleges",
            ...
            "transcript": [
                ...
                r"(?:official\s+)?reporter\'s\s+transcript",
            ],
        }
        default_legal_domain_patterns = {
            "criminal_law": [
                r"defendant\s+charged",
                r"criminal\s+case",
                r"prosecution",
                r"indictment",
                r"sentencing",
        ...
        self.document_type_patterns = self.config.get(
            "document_type_patterns", default_doc_type_patterns
        )
        self.legal_domain_patterns = self.config.get(
            "legal_domain_patterns", default_legal_domain_patterns
        )
        # For brevity, assuming other pattern sets are similarly loaded or default.
        self.procedural_stage_patterns = self.config.get(
            "procedural_stage_patterns",
            {
                "pleading": [r"complaint", r"answer"],
                "discovery": [r"interrogatories", r"deposition"],
            },
        )
        self.importance_indicators = self.config.get(
            "importance_indicators",
            {
                "high": [r"constitutional", r"supreme\s+court"],
                "medium": [r"significant"],
                "low": [r"minor"],
            },
        )
        self.entity_patterns_for_tags = self.config.get(
            "entity_patterns_for_tags",
            {"court": [r"supreme\s+court"], "judge": [r"judge\s+\w+"]},
        )
        self.subject_tags_patterns = self.config.get(
            "subject_tags_patterns", [r"breach\s+of\s+contract", r"due\s+process"]
        )

        self.logger.debug(
            "Tagging frameworks initialized (patterns loaded from config/defaults)."
        )
    ...
    async def _load_learning_data_from_umm_async(self):
        ...
        if not self.unified_memory_manager:
            self.logger.info(
                "UMM not available, skipping load of learning data for auto-tagging."
            )
            return
        try:
            ...
            self.logger.info(
                "Successfully loaded (placeholder) learning data from UMM for auto-tagging."
            )
        except Exception as e:
            self.logger.error(
                "Failed to load learning data from UMM for auto-tagging.", exception=e
            )
    ...
    async def _process_task(
        ...
        existing_tags_list = metadata.get(
            "existing_tags", []
        )  # Tags already on the document
        user_feedback_data = metadata.get(
            "user_feedback", {}
        )  # e.g., {'doc_id': ..., 'correct_tags': [], 'incorrect_tags': []}

        self.logger.info(
            f"Starting auto-tagging for doc '{document_id}'.",
            parameters={
        ...
        try:
            if (
                user_feedback_data
                and user_feedback_data.get("document_id") == document_id
            ):
                await self._apply_user_feedback(document_id, user_feedback_data)

            tagging_result = await self._perform_comprehensive_tagging_async(
                text_content, document_id, existing_tags_list
            )

            output.document_type_classification = tagging_result.get(
            ...
            output.extracted_entity_tags = tagging_result.get(
                "extracted_entity_tags", {}
            )
            output.generated_subject_tags = tagging_result.get(
                "generated_subject_tags", []
            )
            ...
            output.suggested_new_tags = [
                tag
                for tag in output.all_generated_tags
                if tag not in existing_tags_list
            ]
            output.model_used_for_llm = tagging_result.get("model_used_for_llm")
            output.overall_confidence = self._calculate_overall_tagging_confidence(
                tagging_result
            )

            await self._update_learning_from_session_async(
                document_id, output.all_generated_tags
            )
            ...
            self.logger.error(
                f"AgentProcessingError during auto-tagging for doc '{document_id}'.",
                exception=ape,
                exc_info=True,
            )
            ...
            self.logger.error(
                f"Unexpected error during auto-tagging for doc '{document_id}'.",
                exception=e,
                exc_info=True,
            )
    ...
    async def _perform_comprehensive_tagging_async(
        self,
        text: str,
        doc_id: str,
        ...
        pattern_based_tags: Set[str] = set(
            self._normalize_tag(t) for t in existing_tags
        )
        if doc_type and doc_type.get("type") != "unknown":
            pattern_based_tags.add(f"doc_type:{doc_type['type']}")
        pattern_based_tags.update(f"domain:{domain}" for domain in legal_domains)
        if proc_stage and proc_stage.get("stage"):
            pattern_based_tags.add(f"stage:{proc_stage['stage']}")
        if importance and importance.get("level"):
            pattern_based_tags.add(f"importance:{importance['level']}")

        for entity_type, entities_info_list in entity_tags.items():
            for entity_info in entities_info_list[
                :3
            ]:  # Limit number of entity tags per type
        ...
        if self.enable_llm_tag_generation and self.llm_manager:
            try:
                ...
                llm_generated_formatted_tags, model_used_llm = (
                    await self._llm_generate_additional_tags(
                        text_for_llm,
                        list(pattern_based_tags),
                        doc_id,
                    )
                )
            except Exception as llm_e:
                self.logger.error(
                    f"LLM tag generation failed for doc '{doc_id}'.", exception=llm_e
                )
    ...
    def _classify_document_type_sync(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        for doc_type, patterns in self.document_type_patterns.items():
            ...
            if match_count > 0:
                ...
                type_scores[doc_type] = (match_count / len(patterns)) + (
                    0.1 * (match_count - 1)
                )
        ...
        confidence = min(
            1.0, self.min_pattern_confidence + best_raw_score * 0.5
        )  # Adjust scaling factor
        return (
            {"type": best_type, "confidence": round(confidence, 3)}
            if confidence > self.min_pattern_confidence
            else {"type": "unknown", "confidence": 0.3}
        )
    ...
    def _classify_legal_domain_sync(self, text: str) -> List[str]:
        ...
        for domain, patterns in self.legal_domain_patterns.items():
            match_count = sum(
                1 for p_str in patterns if re.search(p_str, text_lower, re.IGNORECASE)
            )
            if match_count > 0:
                domain_scores[domain] = (match_count / len(patterns)) + (
                    0.05 * match_count
                )  # Small boost for more matches

        # Return top N domains or those above a threshold
        sorted_domains = sorted(domain_scores.items(), key=lambda x: x[1], reverse=True)
        return [d for d, s_val in sorted_domains if s_val > 0.3][
            :3
        ]  # Top 3 domains with min score
    ...
    def _identify_procedural_stage_sync(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        for stage, patterns in self.procedural_stage_patterns.items():
            ...
            if match_count > 0:
                stage_scores[stage] = (match_count / len(patterns)) + (
                    0.1 * (match_count - 1)
                )
        ...
        return (
            {"stage": best_stage, "confidence": round(confidence, 3)}
            if confidence > self.min_pattern_confidence
            else None
        )
    ...
    def _assess_importance_level_sync(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        for level in ["high", "medium", "low"]:
            if level in self.importance_indicators:
                if any(
                    re.search(ind_pat, text_lower, re.IGNORECASE)
                    for ind_pat in self.importance_indicators[level]
                ):
    ...
    def _extract_entity_tags_sync(self, text: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for entity_type_label, patterns in self.entity_patterns_for_tags.items():
            ...
            for p_str in patterns:
                try:
                    for match in re.finditer(
                        p_str, text_snippet_for_entities, re.IGNORECASE | re.DOTALL
                    ):
                        ...
                        if norm_name_key not in seen_for_type:
                            current_type_tags.append(
                                {
                                    "name": entity_name,
                                    "confidence": round(
                                        self.min_pattern_confidence + 0.2, 3
                                    ),
                                }
                            )  # Base confidence for entity tag
                            seen_for_type.add(norm_name_key)
                except re.error as re_err:
                    self.logger.warning(
                        f"Regex error during entity tag pattern matching for type '{entity_type_label}'.",
            ...
            entity_tags_map[entity_type_label] = current_type_tags[
                :5
            ]  # Limit to top 5 distinct entities per type
    ...
    async def _llm_generate_additional_tags(
        self, text_for_llm: str, existing_tags: List[str], doc_id: str
    ) -> Tuple[List[str], Optional[str]]:
        if not self.llm_manager:
            return [], None
        self.logger.debug(f"Using LLM for additional tag generation on doc '{doc_id}'.")

        existing_tags_str = ", ".join(
            sorted(list(set(existing_tags)))[:20]
        )  # Unique, sorted, limited
        prompt = f"""
        ...
        model_to_use = self.config.get(
            "llm_model_for_autotag", self.llm_manager.primary_config.model
        )
        provider_to_use = self.config.get(
            "llm_provider_for_autotag", self.llm_manager.primary_config.provider.value
        )
        ...
        try:
            llm_response = await self.llm_manager.complete(
                ...
                model_params={
                    "temperature": 0.4,
                    "max_tokens": 150,
                },  # Temperature slightly higher for creativity
            )
            ...
            if response_text:
                ...
                for tag_candidate in potential_tags:
                    ...
                    tag_candidate = re.sub(
                        r"[^a-z0-9_:]", "", tag_candidate
                    )  # Allow colons for existing prefixes
                    is_covered = any(
                    ...
                    if (
                        tag_candidate
                        and len(tag_candidate) > 2
                        and tag_candidate not in existing_tags
                    ):
                        if not is_covered:
                            llm_formatted_tags.append(f"llm:{tag_candidate}")

            self.logger.debug(
                f"LLM ({model_used_str}) generated {len(llm_formatted_tags)} additional tags for doc '{doc_id}'."
            )
            return llm_formatted_tags[:7], model_used_str  # Limit number of LLM tags
        except LLMProviderError as e:
            self.logger.error(
                f"LLM API call for tag generation failed for doc '{doc_id}'.",
                exception=e,
            )
            raise AgentProcessingError(
                "LLM tag generation failed.", underlying_exception=e
            ) from e
        except Exception as e:
            self.logger.error(
                f"Unexpected error in LLM tag generation for doc '{doc_id}'.",
                exception=e,
            )
            raise AgentProcessingError(
                "Unexpected error during LLM tag generation.", underlying_exception=e
            ) from e

    # --- Learning System Methods ---
    async def _apply_user_feedback(self, doc_id: str, feedback: Dict[str, Any]):
        self.logger.info(
            f"Applying user feedback to learning model (UMM) for doc '{doc_id}'."
        )
        self.feedback_session_log.append(
            {
                "doc_id": doc_id,
                "feedback": feedback,
        ...
        correct_tags = [
            self._normalize_tag(t) for t in feedback.get("correct_tags", [])
        ]
        incorrect_tags = [
            self._normalize_tag(t) for t in feedback.get("incorrect_tags", [])
        ]
        added_by_user = [
            self._normalize_tag(t) for t in feedback.get("added_by_user", [])
        ]
        ...
        if self.unified_memory_manager:
            ...
            for tag in correct_tags:
                tasks.append(
                    self.unified_memory_manager.update_tag_learning_stats_async(
                        tag, correct_increment=1.0, last_updated_ts=ts
                    )
                )
            ...
            for tag in incorrect_tags:
                tasks.append(
                    self.unified_memory_manager.update_tag_learning_stats_async(
                        tag, incorrect_increment=1.0, last_updated_ts=ts
                    )
                )
            ...
            for (
                ...
                added_by_user
            ):  # Tags user added are considered "correct" and "suggested" if new
                tasks.append(
                    self.unified_memory_manager.update_tag_learning_stats_async(
                        tag,
                        correct_increment=1.0,
            ...
            self.logger.debug(
                f"User feedback for doc '{doc_id}' persisted to UMM and local cache updated."
            )
            ...
            for tag in added_by_user:
                ...
                self.tag_accuracy_scores_cache[tag]["last_updated_ts"] = ts
            self.logger.warning(
                "UMM not available. Feedback for auto-tagging applied to in-memory cache only."
            )
    ...
    async def _update_learning_from_session_async(
        self, doc_id: str, generated_tags: List[str]
    ):
        ...
        if self.unified_memory_manager:
            ...
            for tag in normalized_tags:
                tasks.append(
                    self.unified_memory_manager.update_tag_learning_stats_async(
                        tag, suggested_increment=1.0, last_updated_ts=ts
                    )
                )
                self.tag_accuracy_scores_cache[tag]["suggested"] += 1.0
                self.tag_accuracy_scores_cache[tag]["last_updated_ts"] = ts
            if tasks:
                await asyncio.gather(*tasks)
            self.logger.trace(
                f"Learning from current auto-tagging session (suggestion counts) persisted to UMM for doc '{doc_id}'."
            )
            ...
            for tag in normalized_tags:
                ...
                self.tag_accuracy_scores_cache[tag]["last_updated_ts"] = ts
            self.logger.warning(
                "UMM not available. Learning from auto-tagging session applied to in-memory cache only."
            )

    def _calculate_overall_tagging_confidence(
        self, tagging_result: Dict[str, Any]
    ) -> float:
        confidences = []
        if tagging_result.get("document_type_classification") and isinstance(
            tagging_result["document_type_classification"], dict
        ):
            confidences.append(
                tagging_result["document_type_classification"].get("confidence", 0.0)
            )
        if tagging_result.get("procedural_stage_tag") and isinstance(
            tagging_result["procedural_stage_tag"], dict
        ):
            confidences.append(
                tagging_result["procedural_stage_tag"].get("confidence", 0.0)
            )
        if tagging_result.get("importance_level_tag") and isinstance(
            tagging_result["importance_level_tag"], dict
        ):
            confidences.append(
                tagging_result["importance_level_tag"].get("confidence", 0.0)
            )

        # Add base confidence for presence of other tag types
        if tagging_result.get("legal_domain_tags"):
            confidences.append(0.6)
        if tagging_result.get("extracted_entity_tags"):
            confidences.append(0.65)
        if tagging_result.get("generated_subject_tags"):
            confidences.append(0.55)
        if any(
            tag.startswith("llm:")
            for tag in tagging_result.get("all_generated_tags", [])
        ):
            confidences.append(self.min_llm_tag_confidence)

        if not confidences:
            return 0.25  # Low base if nothing classified
    ...
    async def get_learning_statistics_async(self) -> Dict[str, Any]:  # Public method
        ...
        total_correct_feedback = sum(
            stats.get("correct", 0) for stats in cached_stats.values()
        )

        return {
            "feedback_sessions_logged_this_instance": len(self.feedback_session_log),
            "distinct_tags_in_cache": len(cached_stats),
            "overall_cached_tag_accuracy": (
                (total_correct_feedback / total_feedback_events)
                if total_feedback_events > 0
                else 0.0
            ),
            "top_correct_tags_cache": sorted(
                [
                    (tag, data)
                    for tag, data in cached_stats.items()
                    if data.get("correct", 0) > 0
                ],
                key=lambda x: x[1].get("correct", 0),
                reverse=True,
            )[:10],
            "top_incorrect_tags_cache": sorted(
                [
                    (tag, data)
                    for tag, data in cached_stats.items()
                    if data.get("incorrect", 0) > 0
                ],
                key=lambda x: x[1].get("incorrect", 0),
                reverse=True,
            )[:10],
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        status["dependencies_status"] = {
            ...
            "unified_memory_manager": (
                "available" if self.unified_memory_manager else "unavailable"
            ),
        ...
        if self.enable_llm_tag_generation and not self.llm_manager:
            ...
            status["reason"] = (
                "LLM tag generation enabled but LLMManager is unavailable."
            )
        return status
