...
class DocumentContentType(Enum):
    ...
    DOC = "application/msword"; TXT = "text/plain"; MD = "text/markdown"; HTML = "text/html"
    RTF = "application/rtf"; XLSX = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    XLS = "application/vnd.ms-excel"; CSV = "text/csv"; PPTX = "application/vnd.openxmlformats-officedocument.presentationml.presentation"
    PPT = "application/vnd.ms-powerpoint"
    PNG = "image/png"; JPG = "image/jpeg"; TIFF = "image/tiff"; BMP = "image/bmp"
    UNKNOWN = "application/octet-stream"

    @classmethod
    def from_extension(cls, ext: str) -> 'DocumentContentType':
        ext_map = {
            '.pdf': cls.PDF, '.docx': cls.DOCX, '.doc': cls.DOC, '.txt': cls.TXT, '.md': cls.MD,
            '.html': cls.HTML, '.htm': cls.HTML, '.rtf': cls.RTF, '.xlsx': cls.XLSX, '.xls': cls.XLS,
            '.csv': cls.CSV, '.pptx': cls.PPTX, '.ppt': cls.PPT, '.png': cls.PNG, '.jpg': cls.JPG,
            '.jpeg': cls.JPG, '.tiff': cls.TIFF, '.tif': cls.TIFF, '.bmp': cls.BMP
        }
    ...
    def from_mimetype(cls, mime: Optional[str]) -> 'DocumentContentType':
        ...
        if 'word' in mime: return cls.DOCX # Prefer modern if unsure
        if 'excel' in mime or 'spreadsheet' in mime: return cls.XLSX
        if 'presentation' in mime: return cls.PPTX
        return cls.UNKNOWN
...
class DocumentProcessingOutput:
    ...
    extracted_metadata: Dict[str, Any] = field(default_factory=dict) # From file itself + file system
    page_count: Optional[int] = None
    word_count: Optional[int] = None
    character_count: Optional[int] = None
    document_structure: Optional[Dict[str, Any]] = None # e.g., headings, sections

    tables_extracted: Optional[List[Dict[str, Any]]] = None # List of tables, each table a list of lists or dict
    table_count: Optional[int] = None
    
    image_ocr_results: Optional[List[Dict[str,str]]] = None # For files with embedded images + OCR
    ...
    def to_dict(self) -> Dict[str, Any]:
        data = asdict(self)
        data['document_content_type'] = self.document_content_type.value # Store enum value
        return data
...
class DocumentProcessorAgent(BaseAgent, MemoryMixin):
    ...
    def __init__(self, service_container: Any, **config: Any):
        ...
        self.logger.info(f"DocumentProcessorAgentAgent configured with model: {self.llm_config.get('llm_model', 'default')}")
        ...
        self.chunker = DocumentChunker(chunk_size=self.config.get('dp_chunk_size', 4000), 
                                       overlap=self.config.get('dp_chunk_overlap', 400))


        # Configuration
        self.max_file_size_mb = float(self.config.get('max_file_size_mb', Constants.Document.MAX_DOCUMENT_SIZE_MB))
        self.ocr_language = str(self.config.get('ocr_language', 'eng'))
        self.ocr_dpi = int(self.config.get('ocr_dpi', 300))
        self.extract_images_from_pdf = bool(self.config.get('extract_images_from_pdf', False)) # Usually not needed for text processing
        self.clean_extracted_text = bool(self.config.get('clean_extracted_text', True))
        self.preserve_document_structure = bool(self.config.get('preserve_document_structure', False)) # Default to False if not fully supported
        self.extract_tables_from_docs = bool(self.config.get('extract_tables_from_docs', True))
    ...
    def _initialize_file_type_configs(self):
        ...
        self.file_type_configs: Dict[DocumentContentType, Dict[str, Any]] = {
            DocumentContentType.PDF: {'strategy': ProcessingStrategy.FULL_PROCESSING, 'handler_method': '_process_pdf_async', 'deps': ['pymupdf']},
            DocumentContentType.DOCX: {'strategy': ProcessingStrategy.FULL_PROCESSING, 'handler_method': '_process_docx_async', 'deps': ['docx']},
            DocumentContentType.DOC: {'strategy': ProcessingStrategy.FULL_PROCESSING, 'handler_method': '_process_doc_async', 'deps': ['docx']}, # Often python-docx can handle .doc via Word interop or conversion if available
            DocumentContentType.TXT: {'strategy': ProcessingStrategy.FULL_PROCESSING, 'handler_method': '_process_txt_async', 'deps': []},
            DocumentContentType.MD: {'strategy': ProcessingStrategy.FULL_PROCESSING, 'handler_method': '_process_markdown_async', 'deps': ['markdown', 'bs4']},
            DocumentContentType.HTML: {'strategy': ProcessingStrategy.FULL_PROCESSING, 'handler_method': '_process_html_async', 'deps': ['bs4']},
            DocumentContentType.RTF: {'strategy': ProcessingStrategy.FULL_PROCESSING, 'handler_method': '_process_rtf_async', 'deps': ['striprtf']},
            DocumentContentType.XLSX: {'strategy': ProcessingStrategy.STRUCTURED_DATA, 'handler_method': '_process_excel_async', 'deps': ['pandas', 'openpyxl']},
            DocumentContentType.XLS: {'strategy': ProcessingStrategy.STRUCTURED_DATA, 'handler_method': '_process_excel_async', 'deps': ['pandas', 'xlrd']}, # xlrd for older .xls
            DocumentContentType.CSV: {'strategy': ProcessingStrategy.STRUCTURED_DATA, 'handler_method': '_process_csv_async', 'deps': ['pandas']},
            DocumentContentType.PPTX: {'strategy': ProcessingStrategy.REFERENCE_ONLY, 'handler_method': '_process_powerpoint_async', 'deps': ['pptx']},
            DocumentContentType.PPT: {'strategy': ProcessingStrategy.REFERENCE_ONLY, 'handler_method': '_process_powerpoint_async', 'deps': ['pptx']}, # python-pptx does not handle .ppt directly
            DocumentContentType.PNG: {'strategy': ProcessingStrategy.OCR_IF_NEEDED, 'handler_method': '_process_image_async', 'deps': ['pytesseract', 'PIL']},
            DocumentContentType.JPG: {'strategy': ProcessingStrategy.OCR_IF_NEEDED, 'handler_method': '_process_image_async', 'deps': ['pytesseract', 'PIL']},
            DocumentContentType.TIFF: {'strategy': ProcessingStrategy.OCR_IF_NEEDED, 'handler_method': '_process_image_async', 'deps': ['pytesseract', 'PIL']},
            DocumentContentType.BMP: {'strategy': ProcessingStrategy.OCR_IF_NEEDED, 'handler_method': '_process_image_async', 'deps': ['pytesseract', 'PIL']},
        }

    def get_config_summary_params(self) -> Dict[str,Any]:
        return {
            'max_file_size_mb': self.max_file_size_mb, 'ocr_lang': self.ocr_language, 'ocr_dpi': self.ocr_dpi,
    ...
    @detailed_log_function(LogCategory.DOCUMENT_PROCESSING) # Use agent's logger with category
    async def _process_task(self, task_data: Union[str, Path], metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if not file_path.is_file():
                raise DocumentProcessingError(f"Path is not a file: {file_path}", file_path=file_path)

            if (output.file_size_bytes / (1024*1024)) > self.max_file_size_mb:
                raise DocumentProcessingError(f"File '{file_path.name}' ({output.file_size_bytes / (1024*1024):.2f}MB) "
                                              f"exceeds max size of {self.max_file_size_mb}MB.", file_path=file_path)
            ...
            if output.document_content_type == DocumentContentType.UNKNOWN:
                ...
                output.document_content_type = DocumentContentType.from_mimetype(mime_type_str)
                self.logger.debug(f"Used MIME type '{mime_type_str}' to determine content type: {output.document_content_type.name}",
            ...
            if not file_handler_config:
                raise DocumentProcessingError(f"Unsupported file type: {file_type_enum.name} (ext: {file_ext})", file_path=file_path)
            ...
            for dep_key in file_handler_config.get('deps', []):
                if not dep_manager.is_available(dep_key):
                    dep_info = dep_manager.DEPENDENCY_MAP.get(dep_key, {'package': f"unknown_package_for_{dep_key}"})
                    raise DocumentProcessingError(f"Missing dependency '{dep_info['package']}' required for {file_type_enum.name} files.", file_path=file_path)
            ...
            if not handler_method or not callable(handler_method):
                raise DocumentProcessingError(f"Internal error: Handler method '{handler_method_name}' not found or not callable.", file_path=file_path)
            
            # Pass relevant processing options to the handler method
            processing_options = {
                'force_ocr': metadata.get('force_ocr', False), # Metadata can override agent defaults
                'ocr_language': self.ocr_language, 'ocr_dpi': self.ocr_dpi,
                ...
                'metadata_extraction_level': metadata.get('metadata_extraction_level', 'full') # 'basic' or 'full'
            }
            ...
            output.extracted_metadata.update(handler_result.get('extracted_metadata', {}))
            ...
            output.table_count = len(output.tables_extracted) if output.tables_extracted else handler_result.get('table_count',0)
            ...
            if output.text_content:
                ...
                output.content_hash_sha256 = hashlib.sha256(output.text_content.encode('utf-8', 'ignore')).hexdigest()
                
                # Perform classification using the shared LegalDocumentClassifier
                classification_result = self.classifier.classify(output.text_content, filename=file_path.name)
                output.is_legal_document_classified = classification_result.get('is_legal_document', False)
                output.classification_details = classification_result
                output.processing_notes.append(f"Document classification: Type='{classification_result.get('primary_type', 'N/A')}', Score={classification_result.get('primary_score', 0.0):.2f}")
            ...
            self.logger.error(f"AgentExecutionError during processing of '{file_path.name}': {str(aee)}",
                             parameters={'file': str(file_path)}, exception=aee) # Log full exception if needed
            output.errors.append(f"Agent execution error: {str(aee)}")
        except Exception as e:
            self.logger.error(f"Unexpected error processing '{file_path.name}'.", 
    ...
    async def _process_pdf_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_pdf, file_path, options)
    ...
    async def _process_markdown_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_markdown, file_path, options)
    async def _process_html_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_html, file_path, options)
    async def _process_rtf_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_rtf, file_path, options)
    async def _process_excel_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_excel, file_path, options)
    async def _process_csv_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_csv, file_path, options)
    async def _process_powerpoint_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_powerpoint, file_path, options)
    async def _process_image_async(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.get_event_loop().run_in_executor(None, self._sync_process_image, file_path, options)
    ...
    def _common_text_cleaning(self, text: Optional[str]) -> Optional[str]:
        ...
        text = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\x9f]', '', text) # Remove most control characters except \t, \n, \r
        return text.strip()
    ...
    def _sync_process_pdf(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for k, v in meta.items(): # Clean metadata values
                if isinstance(v, str) and v.startswith("D:"): meta[k] = v[2:] # Clean PDF date format
            extracted_meta.update(meta)
            extracted_meta['page_count_from_meta'] = page_count # Store original page count from metadata

            for i in range(page_count):
                page = doc.load_page(i)
                page_text = page.get_text("text", sort=True).strip() # Added sort=True for reading order
                
                # OCR attempt if no text or forced
                needs_ocr = options.get('force_ocr', False) or (not page_text and options.get('ocr_if_needed', True))
                if needs_ocr:
                    if dep_manager.is_available('pytesseract') and dep_manager.is_available('PIL'):
                        notes.append(f"Page {i+1}: Attempting OCR (Reason: {'forced' if options.get('force_ocr') else 'no text layer'}).")
                        try:
                            pix = page.get_pixmap(dpi=options.get('ocr_dpi', 300))
                            img_bytes = pix.tobytes("png") # Get bytes in a common format
                            ...
                            if ocr_text:
                                page_text = ocr_text # Replace page_text with OCR result
                                notes.append(f"Page {i+1}: OCR successful, added {len(ocr_text)} chars.")
                                image_ocr_res.append({'page': str(i+1), 'ocr_text_preview': ocr_text[:100]+"..."})
                            else:
                                notes.append(f"Page {i+1}: OCR yielded no text.")
                        except Exception as ocr_e:
                            notes.append(f"Page {i+1}: OCR failed - {str(ocr_e)[:100]}.") # Limit error message length
                            file_logger.warning(f"OCR for page {i+1} of {file_path.name} failed.", exception=ocr_e)
                    else:
                        notes.append(f"Page {i+1}: OCR skipped (pytesseract/Pillow unavailable).")
                
                if page_text: text_content_parts.append(page_text)
            doc.close()
        except Exception as e:
        ...
        if options.get('clean_text', True): full_text = self._common_text_cleaning(full_text) or ""
        
        return {
            "text_content": full_text, "extracted_metadata": extracted_meta,
            "page_count": page_count, "processing_notes": notes,
    ...
    def _sync_process_docx(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for prop_name in ['title', 'author', 'subject', 'keywords', 'comments', 'category', 'last_modified_by']:
                val = getattr(cp, prop_name, None)
                if val: extracted_meta[prop_name] = str(val)
            if cp.created: extracted_meta['created_date'] = cp.created.isoformat()
            if cp.modified: extracted_meta['modified_date'] = cp.modified.isoformat()
            if cp.revision: extracted_meta['revision'] = cp.revision
        ...
        return {
            "text_content": full_text, "extracted_metadata": extracted_meta,
            "processing_notes": notes, "tables_extracted": tables_data if tables_data else None,
    ...
    def _sync_process_txt(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        content = None; encoding_used = "unknown"; notes = []

        for enc in encodings_to_try:
            try:
                with open(file_path, 'r', encoding=enc) as f: content = f.read()
                encoding_used = enc; notes.append(f"Read with encoding: {enc}"); break
            except (UnicodeDecodeError, LookupError): continue # LookupError for invalid encoding name
        
        if content is None:
            try: # Fallback with error replacement
                with open(file_path, 'rb') as fb: raw_bytes = fb.read()
    ...
    def _sync_process_markdown(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if dep_manager.is_available('markdown') and dep_manager.is_available('bs4'):
            try:
                ...
                return self._sync_process_txt(file_path, options) # Fallback to plain text reading
        else:
            notes.append("Markdown/BeautifulSoup unavailable. Reading as plain text.")
            return self._sync_process_txt(file_path, options)

        if options.get('clean_text', True): text_content = self._common_text_cleaning(text_content) or ""
        return {"text_content": text_content, "extracted_metadata": {"source_format": "markdown"}, "processing_notes": notes}
    ...
    def _sync_process_html(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            text_parts = []
            for element in soup.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'div', 'li', 'td', 'th']):
                block_text = element.get_text(separator=' ', strip=True)
                if block_text: text_parts.append(block_text)
            text_content = "\n\n".join(text_parts)

            if not text_content.strip(): # Fallback
                text_content = soup.get_text(separator='\n', strip=True)
            
            extracted_meta = {'source_format': 'html', 'title': soup.title.string if soup.title else None}
        ...
        if options.get('clean_text', True): text_content = self._common_text_cleaning(text_content) or ""
        return {"text_content": text_content, "extracted_metadata": extracted_meta, "processing_notes": notes}

    def _sync_process_rtf(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        file_logger.info(f"Processing RTF (sync): {file_path.name}")
        notes = []; text_content = ""
        if dep_manager.is_available('striprtf'):
            try:
                with open(file_path, 'r', encoding='ascii', errors='ignore') as f: # RTF typically ASCII/extended ASCII
                    rtf_content = f.read()
                text_content = rtf_to_text(rtf_content) # type: ignore
                notes.append("Processed RTF using striprtf library.")
            except Exception as e:
                notes.append(f"Error processing RTF with striprtf: {str(e)}. Fallback to text.")
                return self._sync_process_txt(file_path, options)
        else:
            notes.append("striprtf library not available for RTF. Reading as plain text (may include RTF markup).")
            return self._sync_process_txt(file_path, options)
        
        if options.get('clean_text', True): text_content = self._common_text_cleaning(text_content) or ""
        return {"text_content": text_content, "extracted_metadata": {"source_format": "rtf"}, "processing_notes": notes}

    def _sync_process_excel(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        file_logger.info(f"Processing Excel (sync): {file_path.name}")
        notes = []; all_text_parts: List[str] = []
        tables_data: List[Dict[str, Any]] = []
            ...
            for sheet_idx, sheet_name in enumerate(xls.sheet_names):
                try:
                    ...
                    for col in df.columns:
                        ...
                        except Exception: # Catch errors during astype or string operations
                            notes.append(f"Could not process column '{col}' in sheet '{sheet_name}' fully as text.")
                    ...
                    if options.get('extract_tables', True):
                        ...
                        table_as_list = [df.columns.tolist()] + df.values.astype(str).tolist()
                        tables_data.append({"sheet_name": sheet_name, "table_index_in_doc": sheet_idx, "rows": table_as_list})
                    notes.append(f"Processed sheet: {sheet_name} ({len(df)} rows, {len(df.columns)} cols).")
                except Exception as sheet_e:
                    notes.append(f"Error processing sheet '{sheet_name}': {str(sheet_e)}")
                    file_logger.warning(f"Error processing sheet '{sheet_name}' in {file_path.name}.", exception=sheet_e)
        ...
        return {
            "text_content": full_text if options.get('processing_strategy_used') == ProcessingStrategy.FULL_PROCESSING else None, # Text only if full processing
            "extracted_metadata": extracted_meta, "processing_notes": notes,
            "tables_extracted": tables_data if tables_data else None,
            "table_count": len(tables_data)
        }

    def _sync_process_csv(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for enc in encodings_to_try:
                try:
                    ...
                    notes.append(f"Pandas parsing error with encoding {enc}: {str(pe)}. Trying next.")
            ...
            if df.empty:
                ...
                if options.get('extract_tables', True):
                    table_as_list = [df.columns.tolist()] + df.values.astype(str).tolist()
                    tables_data.append({"table_index_in_doc": 0, "rows": table_as_list}) # CSV is one table
        ...
        return {
            "text_content": full_text if options.get('processing_strategy_used') == ProcessingStrategy.FULL_PROCESSING else None,
            "extracted_metadata": extracted_meta, "processing_notes": notes,
            "tables_extracted": tables_data if tables_data else None,
            "table_count": len(tables_data)
        }

    def _sync_process_powerpoint(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        file_logger.info(f"Processing PowerPoint (sync): {file_path.name}")
        notes = []; text_parts: List[str] = []
        slide_count = 0
        ...
        try:
            ...
            for i, slide in enumerate(prs.slides):
                ...
                for shape in slide.shapes:
                    ...
                    elif hasattr(shape, "has_table") and shape.has_table and options.get('extract_tables', True):
    ...
    def _sync_process_image(self, file_path: Path, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            extracted_meta.update({'format': pil_img.format, 'mode': pil_img.mode, 
                                   'size': pil_img.size, 'info': pil_img.info}) # pil_img.info can be large

            # Pre-processing (optional, can improve OCR)
            # img = img.convert('L') # Convert to grayscale
            # img = img.filter(ImageFilter.MedianFilter()) # Example filter
            
            text_content = pytesseract.image_to_string(pil_img, lang=options.get('ocr_language', 'eng'),
                                                     config=f"--dpi {options.get('ocr_dpi',300)}") # type: ignore
            text_content = text_content.strip()
            if text_content:
                notes.append(f"OCR successful, extracted {len(text_content)} characters.")
            else:
    ...
    async def health_check(self) -> Dict[str, Any]: # Public method
        ...
        for content_type, config_details in self.file_type_configs.items():
            deps_ok = all(dep_manager.is_available(dep) for dep in config_details.get('deps', []))
            if deps_ok:
                supported_formats_with_deps.append(content_type.name)
            else:
                missing = [dep for dep in config_details.get('deps',[]) if not dep_manager.is_available(dep)]
                unsupported_due_to_deps.append(f"{content_type.name} (missing: {', '.join(missing)})")
        ...
        if unsupported_due_to_deps: # If some formats are not supported due to missing deps
            base_status['status'] = 'degraded' # Could be 'warning' depending on severity definition
            base_status['reason'] = f"Some file formats unsupported due to missing dependencies: {len(unsupported_due_to_deps)} types."
