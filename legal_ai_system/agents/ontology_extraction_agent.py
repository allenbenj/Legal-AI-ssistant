...
class OntologyExtractionAgent(BaseAgent):
    ...
    def __init__(self, services: Any, **config: Any):
        ...
        self.logger.info(f"OntologyExtractionAgentAgent configured with model: {self.llm_config.get('llm_model', 'default')}")
        ...
        if self.enable_regex_extraction:
            self.entity_patterns = self._load_patterns_from_file(self.entity_patterns_path, "entity")
            self.relationship_patterns = self._load_patterns_from_file(self.relationship_patterns_path, "relationship")

        self.logger.info(f"{self.name} initialized. Config: {self.get_agent_config_summary()}")

    def _load_agent_config(self):
        """Loads various configuration parameters for the agent."""
        self.confidence_threshold_final = float(self.config.get('confidence_threshold_final', 0.6))
        self.confidence_threshold_regex = float(self.config.get('confidence_threshold_regex', 0.4))
        self.confidence_threshold_llm = float(self.config.get('confidence_threshold_llm', 0.7))
        self.max_entities_per_type = int(self.config.get('max_entities_per_type', 30))
        self.max_text_chunk_size_for_llm = int(self.config.get('max_text_chunk_size_for_llm', 4000)) # Characters for LLM input

        self.enable_regex_extraction = bool(self.config.get('enable_regex_extraction', True))
        self.enable_llm_extraction = bool(self.config.get('enable_llm_extraction', True))
        
        # Deduplication config
        self.enable_advanced_deduplication = bool(self.config.get('enable_advanced_deduplication', True))
        self.deduplication_span_overlap_threshold = float(self.config.get('deduplication_span_overlap_threshold', 0.7))
        self.deduplication_semantic_similarity_threshold = float(self.config.get('deduplication_semantic_similarity_threshold', 0.80))
        if self.enable_advanced_deduplication and not self.embedding_manager:
            self.logger.warning("EmbeddingManager service not found, but semantic deduplication is enabled. "
                                "Semantic part of deduplication will be skipped.")
        
        # Coreference config
        self.enable_coreference_resolution = bool(self.config.get('enable_coreference_resolution', True))
        self.coref_model_name = self.config.get('coref_model_name', 'en_coreference_web_trf')
        
        # Pattern file paths
        # Assuming a 'config_base_path' might be in self.config or resolved globally
        base_path = Path(self.config.get('config_base_path', '.')) # Default to current dir if not set
        self.entity_patterns_path = base_path / self.config.get('entity_patterns_file', 'config/ontology_extraction/entity_patterns.yaml')
        self.relationship_patterns_path = base_path / self.config.get('relationship_patterns_file', 'config/ontology_extraction/relationship_patterns.yaml')
    ...
    def _initialize_coref_model(self):
        ...
        try:
            self.nlp_coref = spacy.load(self.coref_model_name, disable=["tagger", "parser", "ner", "lemmatizer", "attribute_ruler"])
            # Verify coref component exists
            pipe_names = self.nlp_coref.pipe_names
            if not ('coref' in pipe_names or 'experimental_coref' in pipe_names or 'neuralcoref' in pipe_names): # Check for variations
                raise ConfigurationError(f"Coreference component (e.g., 'coref', 'experimental_coref') not found in spaCy model '{self.coref_model_name}'. Model pipes: {pipe_names}")
            self.logger.info(f"Coreference resolution model '{self.coref_model_name}' loaded successfully.")
        except OSError as e:
            self.logger.error(f"Could not load spaCy coreference model '{self.coref_model_name}'. "
                              "Ensure it's downloaded (e.g., python -m spacy download en_coreference_web_trf). Disabling coreference.", exc_info=False)
            self.logger.debug("Detailed spaCy load error:", exc_info=True) # Debug level for full trace
            self.enable_coreference_resolution = False
            self.nlp_coref = None
        except Exception as e: # Catch other potential errors during model loading
            self.logger.error(f"Unexpected error loading coreference model '{self.coref_model_name}': {e}. Disabling coreference.", exc_info=True)
            self.enable_coreference_resolution = False
            self.nlp_coref = None

    def _load_patterns_from_file(self, file_path: Path, pattern_type: str) -> Dict[str, List[Dict[str, Any]]]:
        """Loads regex patterns from a YAML file."""
        patterns: Dict[str, List[Dict[str, Any]]] = {}
        if not file_path.is_file():
            self.logger.warning(f"{pattern_type.capitalize()} patterns file not found at {file_path}. No regex patterns of this type will be used.")
            return patterns
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                loaded_patterns = yaml.safe_load(f)
            if not isinstance(loaded_patterns, dict):
                self.logger.error(f"Invalid format in {pattern_type} patterns file {file_path}. Expected a root dictionary. Loaded: {type(loaded_patterns)}")
                return patterns
            
            for type_label, pattern_config_list in loaded_patterns.items():
                if not isinstance(pattern_config_list, list):
                    self.logger.warning(f"Patterns for '{type_label}' in {file_path} is not a list. Skipping.")
                    continue
                ...
                for p_config in pattern_config_list:
                    if isinstance(p_config, dict) and 'pattern' in p_config:
                        ...
                        self.logger.warning(f"Invalid pattern config under '{type_label}' in {file_path}: {p_config}. Missing 'pattern' key or not a dict. Skipping this config.")
                if valid_configs_for_type:
                    patterns[type_label] = valid_configs_for_type
            
            self.logger.info(f"Successfully loaded {sum(len(p_list) for p_list in patterns.values())} "
                             f"{pattern_type} pattern configurations for {len(patterns)} types from {file_path}")
        except yaml.YAMLError as e:
            self.logger.error(f"Error parsing YAML {pattern_type} patterns file {file_path}: {e}", exc_info=True)
        except IOError as e:
            self.logger.error(f"Error reading {pattern_type} patterns file {file_path}: {e}", exc_info=True)
        except Exception as e: # Catch-all for other unexpected errors
            self.logger.error(f"Unexpected error loading {pattern_type} patterns from {file_path}: {e}", exc_info=True)
        return patterns
    ...
    def _get_text_content(self, document: LegalDocument) -> str:
        ...
        if hasattr(document, 'processed_content') and isinstance(document.processed_content, dict):
            text = document.processed_content.get('text')
            if isinstance(text, str) and text.strip():
                return text.strip()
        if hasattr(document, 'content') and isinstance(document.content, str) and document.content.strip():
    ...
    # --- Regex Extraction ---
    def _extract_entities_by_patterns(self, text: str, doc_id: str) -> List[ExtractedEntity]:
        """Extracts entities using pre-loaded regex patterns."""
        extracted_entities: List[ExtractedEntity] = []
        if not self.entity_patterns: return extracted_entities
        ...
        for entity_type_label, patterns_for_type in self.entity_patterns.items():
            ...
            if not entity_type_enum:
                self.logger.warning(f"Unknown entity type label '{entity_type_label}' from pattern config. Skipping.")
                continue
            ...
            for pattern_config in patterns_for_type:
                ...
                try:
                    for match in re.finditer(regex_pattern, text, re.IGNORECASE | re.MULTILINE | re.DOTALL):
                        context_score = self._calculate_context_relevance(text, match.span(), pattern_config.get('context_keywords', []))
                        ...
                        if attributes_map:
                            for group_name, attr_name in attributes_map.items():
                                try:
                                    attributes[attr_name] = match.group(group_name).strip()
                                except IndexError: # Should not happen with named groups if pattern is correct
                                    self.logger.warning(f"Named capture group '{group_name}' not found in match for pattern '{regex_pattern}' of type '{entity_type_label}'.")
                        elif default_attribute_names:
                            if match.groups():
                                for i, attr_name in enumerate(default_attribute_names):
                                    if i < len(match.groups()): attributes[attr_name] = match.group(i+1).strip()
                                    else: break
                            elif default_attribute_names : attributes[default_attribute_names[0]] = match.group(0).strip()
                        else: attributes['matched_text'] = match.group(0).strip()
                    ...
                    self.logger.error(f"Regex error with pattern '{regex_pattern}' for type '{entity_type_label}': {ree}")
        
        self.logger.info(f"Regex entity extraction for doc {doc_id} yielded {len(extracted_entities)} entities.")
        return extracted_entities

    # --- Coreference Resolution ---
    async def _resolve_coreferences(self, text: str, doc_id: str) -> Dict[Tuple[int, int], str]:
        """
        ...
        self.logger.debug(f"Starting coreference resolution for doc {doc_id} (text length: {len(text)} chars)")
        try:
            ...
            self.logger.error(f"Error during spaCy processing for coreference on doc {doc_id}: {e}", exc_info=True)
            return {}
        ...
        clusters_data = None
        potential_coref_keys = ['coref', 'experimental_coref', 'coref_clusters'] # Add others if known
        
        for key in potential_coref_keys:
            if key in spacy_doc.spans:
                clusters_data = spacy_doc.spans[key] # This is a list of SpanGroup objects, each SpanGroup a cluster
                self.logger.debug(f"Using coref clusters from doc.spans['{key}']")
                break
        
        if clusters_data is None and hasattr(spacy_doc._, 'coref_clusters'): # Fallback for neuralcoref
            clusters_data = spacy_doc._.coref_clusters # This is a list of neuralcoref.Cluster objects
            self.logger.debug("Using coref clusters from doc._.coref_clusters (neuralcoref)")

        if clusters_data is None:
            self.logger.warning(f"No coreference cluster data found in spaCy doc for {doc_id} after checking common locations.")
            return {}
        ...
        for i, cluster_group_or_obj in enumerate(clusters_data):
            ...
            if not mentions_in_cluster: continue
            num_clusters += 1
            ...
            for mention_span_obj in mentions_in_cluster:
                mention_span_tuple = (mention_span_obj.start_char, mention_span_obj.end_char)
                mention_to_canonical_id[mention_span_tuple] = canonical_cluster_id
                num_mentions +=1
        
        self.logger.info(f"Coreference resolution for doc {doc_id} identified {num_clusters} clusters, {num_mentions} total mentions.")
        return mention_to_canonical_id

    # --- LLM Extraction ---
    async def _extract_with_llm(self, text: str, doc_id: str, 
        ...
        if not self.llm_manager:
            self.logger.warning(f"LLM provider not available for doc {doc_id}. Skipping LLM extraction.")
            return {'entities': [], 'relationships': []}

        self.logger.debug(f"Initiating LLM extraction for document {doc_id} (text length: {len(text)} chars)")
        
        # Basic chunking if text is too long (a more sophisticated strategy is needed for production)
        chunks = [text[i:i+self.max_text_chunk_size_for_llm] for i in range(0, len(text), self.max_text_chunk_size_for_llm)]
        self.logger.info(f"Text for doc {doc_id} split into {len(chunks)} chunk(s) for LLM processing.")

        all_llm_entities: List[ExtractedEntity] = []
        all_llm_relationships: List[ExtractedRelationship] = []
        
        base_prompt_template = get_extraction_prompt() # Contains structure, entity/rel types

        for chunk_idx, text_chunk in enumerate(chunks):
            self.logger.debug(f"Processing LLM extraction for doc {doc_id}, chunk {chunk_idx+1}/{len(chunks)}")
            ...
            try:
                llm_response_str = await self.llm_manager.complete(
                    prompt=full_prompt,
                    model_params={'temperature': 0.05, 'max_tokens': 3000} # Adjust as needed
                )
                
                parsed_data = self._parse_llm_json_response(llm_response_str)
                if not parsed_data or not isinstance(parsed_data, dict):
                    self.logger.warning(f"LLM response for {doc_id}, chunk {chunk_idx+1} was not a valid JSON object after parsing.")
                ...
                for entity_data in parsed_data.get('entities', []):
                    ...
                    entity_type_enum = get_entity_type_by_label(entity_type_label)
                    if not entity_type_enum:
                        self.logger.warning(f"LLM returned unknown entity type '{entity_type_label}' (doc {doc_id}, chunk {chunk_idx+1}). Skipping.")
                        continue
                    
                    confidence = float(entity_data.get('confidence_score', self.confidence_threshold_llm))
                    if confidence < self.confidence_threshold_llm: continue
                ...
                for rel_data in parsed_data.get('relationships', []):
                    ...
                    if not rel_type_enum:
                        self.logger.warning(f"LLM returned unknown relationship type '{rel_type_label}' (doc {doc_id}, chunk {chunk_idx+1}). Skipping.")
                        continue
                    
                    confidence = float(rel_data.get('confidence_score', self.confidence_threshold_llm))
                    if confidence < self.confidence_threshold_llm: continue

                    # Adjust span relative to the original document
                    chunk_rel_span = rel_data.get('span', (0,0))
                    original_doc_rel_span = (chunk_rel_span[0] + chunk_offset, chunk_rel_span[1] + chunk_offset)
                    ...
                    all_llm_relationships.append(ExtractedRelationship(
                        ...
                        evidence_text_snippet=rel_data.get('evidence_text_snippet', '')[:150].strip(),
                        span=original_doc_rel_span
                    ))
            except Exception as e:
                self.logger.error(f"LLM extraction for doc {doc_id}, chunk {chunk_idx+1} failed: {e}", exc_info=True)
                # Optionally, re-raise as AgentProcessingError or continue with next chunk
        
        self.logger.info(f"LLM extraction for doc {doc_id} (all chunks) yielded {len(all_llm_entities)} entities and {len(all_llm_relationships)} relationships.")
    ...
    def _parse_llm_json_response(self, response_str: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            code_block_match = re.search(r"```(?:json)?\s*(\{.*\}|\[.*\])\s*```", response_str, re.DOTALL | re.IGNORECASE)
            if code_block_match:
                ...
                if first_idx != -1:
                    ...
                    for i in range(first_idx, len(response_str)):
                        if response_str[i] == start_char_found:
                            ...
                            open_count -= 1
                            if open_count == 0: # Found the matching end for the first opening char
                                temp_last_idx = i
            ...
            self.logger.warning(f"Failed to decode JSON from LLM response. Error: {jde}. Response snippet: {json_str[:200] if 'json_str' in locals() else response_str[:200]}")
            return None
        except Exception as e: # Catch any other error during parsing
            self.logger.error(f"Unexpected error parsing LLM JSON response: {e}", exc_info=True)
            return None

    # --- Deduplication ---
    def _get_jaccard_index(self, span1: Tuple[int, int], span2: Tuple[int, int]) -> float:
    ...
    def _merge_entities(self, primary: ExtractedEntity, secondary: ExtractedEntity) -> ExtractedEntity:
        ...
        primary.confidence = max(primary.confidence, secondary.confidence) # Take max confidence
    ...
    async def _post_process_entities_advanced_dedup(self, entities: List[ExtractedEntity]) -> List[ExtractedEntity]:
        """Advanced deduplication: span overlap then semantic similarity."""
        if not entities or not self.enable_advanced_deduplication:
            return entities

        self.logger.debug(f"Starting advanced deduplication for {len(entities)} entities.")
        
        # Group by entity type for type-specific deduplication
        entities_by_type: Dict[str, List[ExtractedEntity]] = {}
        for e in entities: entities_by_type.setdefault(e.entity_type, []).append(e)
        ...
        for entity_type, type_specific_entities in entities_by_type.items():
            ...
            sorted_entities = sorted(type_specific_entities, key=lambda e: (-e.confidence, e.entity_id))
            ...
            for i in range(len(sorted_entities)):
                if merged_by_span[i]: continue
                current_canonical = sorted_entities[i]
                for j in range(i + 1, len(sorted_entities)):
                    if merged_by_span[j]: continue
                    entity_to_compare = sorted_entities[j]
                    if current_canonical.span != (0,0) and entity_to_compare.span != (0,0) and \
                       self._get_jaccard_index(current_canonical.span, entity_to_compare.span) >= self.deduplication_span_overlap_threshold:
                        self.logger.trace(f"Span Merging: '{entity_to_compare.entity_id}' into '{current_canonical.entity_id}' (Type: {entity_type})")
                        current_canonical = self._merge_entities(current_canonical, entity_to_compare)
                        merged_by_span[j] = True
                span_dedup_list.append(current_canonical)
            
            self.logger.debug(f"Type '{entity_type}': Span deduplication {len(type_specific_entities)} -> {len(span_dedup_list)} entities.")
            current_processing_list = span_dedup_list

            # --- Semantic Similarity Deduplication ---
            if self.embedding_manager and len(current_processing_list) > 1:
                texts_to_embed = [e.source_text_snippet for e in current_processing_list] # Or more representative text from attributes
                try:
                    ...
                    if embeddings_list and len(embeddings_list) == len(texts_to_embed):
                        ...
                        for i in range(len(current_processing_list)):
                            if merged_by_semantic[i]: continue
                            current_semantic_canonical = current_processing_list[i] # Get from original order for matrix indexing
                            
                            for j in range(i + 1, len(current_processing_list)):
                                if merged_by_semantic[j]: continue
                                if similarity_matrix[i, j] >= self.deduplication_semantic_similarity_threshold:
                                    entity_to_compare_semantic = current_processing_list[j]
                                    self.logger.trace(f"Semantic Merging (Sim: {similarity_matrix[i,j]:.3f}): "
                                    ...
                                    if entity_to_compare_semantic.confidence > current_semantic_canonical.confidence:
                                        current_semantic_canonical, entity_to_compare_semantic = entity_to_compare_semantic, current_semantic_canonical
                                    ...
                                    current_semantic_canonical = self._merge_entities(current_semantic_canonical, entity_to_compare_semantic)
                                    merged_by_semantic[j] = True 
                            temp_semantic_list.append(current_semantic_canonical)
                        
                        current_processing_list = temp_semantic_list
                        self.logger.debug(f"Type '{entity_type}': Semantic deduplication -> {len(current_processing_list)} entities.")
                    else:
                        self.logger.warning(f"Failed to get valid embeddings for semantic deduplication (Type: {entity_type}). Skipping semantic step.")
                except Exception as e:
                    self.logger.error(f"Error during semantic deduplication (Type: {entity_type}): {e}", exc_info=True)
            
            final_deduplicated_entities.extend(current_processing_list)
        
        self.logger.info(f"Advanced deduplication complete. Total entities: {len(final_deduplicated_entities)}.")
        return final_deduplicated_entities

    # --- Relationship Processing ---
    def _validate_relationships(self, relationships: List[ExtractedRelationship], 
                              final_entities_map: Dict[str, ExtractedEntity]) -> List[ExtractedRelationship]:
        ...
        for rel in relationships:
            ...
            if source_valid and target_valid and rel.source_entity_id != rel.target_entity_id:
                ...
                self.logger.debug(f"Invalidating relationship {rel.relationship_id} ({rel.relationship_type}): "
    ...
    def _post_process_relationships(self, relationships: List[ExtractedRelationship]) -> List[ExtractedRelationship]:
        """Post-processes relationships: filter by confidence, deduplicate."""
        if not relationships: return []
        
        confident_rels = [r for r in relationships if r.confidence >= self.confidence_threshold_final]
        ...
        for rel in sorted(confident_rels, key=lambda r: (-r.confidence, r.relationship_id)):
            # Create a more unique key, e.g., including a primary property if available
            primary_prop_key = ""
            if rel.properties: # Example: use sorted string of key-values from properties
                primary_prop_key = "_".join(sorted([f"{k}-{v}" for k,v in rel.properties.items()]))

            dedup_key = (rel.relationship_type, rel.source_entity_id, rel.target_entity_id, primary_prop_key)
        ...
        self.logger.debug(f"Post-processing relationships: {len(relationships)} -> {len(confident_rels)} (confident) -> {len(final_rels)} (unique)")
        return final_rels

    # --- Orchestration: `process` method ---
    async def process(self, document: LegalDocument, metadata: Optional[Dict[str, Any]] = None) -> ProcessingResult:
        ...
        try:
            ...
            if not text_content:
                ...
                return ProcessingResult(success=False, error="No text content for extraction.", data=output.to_dict())
            output.extraction_metadata['text_length_chars'] = len(text_content)

            # 1. Coreference Resolution
            mention_to_canonical_id_map: Dict[Tuple[int, int], str] = {}
            if self.enable_coreference_resolution:
                output.extraction_metadata['extraction_methods_used'].append('coreference_resolution')
                mention_to_canonical_id_map = await self._resolve_coreferences(text_content, document.id)
                output.extraction_metadata['coreference_clusters_found'] = len(set(mention_to_canonical_id_map.values()))

            # 2. Initial Entity Extraction (Regex + LLM)
            initial_entities: List[ExtractedEntity] = []
            initial_relationships: List[ExtractedRelationship] = [] # Primarily from LLM

            if self.enable_regex_extraction:
                output.extraction_metadata['extraction_methods_used'].append('regex_entity_extraction')
            ...
            if self.enable_llm_extraction and self.llm_manager:
                ...
                initial_relationships.extend(llm_extracted_data.get('relationships', []))
            ...
            for entity in initial_entities:
                if not entity.canonical_entity_id and entity.span in mention_to_canonical_id_map:
                    entity.canonical_entity_id = mention_to_canonical_id_map[entity.span]
                elif not entity.canonical_entity_id: # Not in a coref cluster, it's its own canonical
            ...
            for canon_id, mention_list in entities_grouped_by_canonical_id.items():
                if not mention_list: continue
                # Choose the "best" mention as the canonical representative (e.g., highest confidence, longest snippet)
                mention_list.sort(key=lambda e: (-e.confidence, -len(e.source_text_snippet), e.entity_id))
                ...
                for other_mention in mention_list[1:]:
                    canonical_mention_obj = self._merge_entities(canonical_mention_obj, other_mention) # Modifies canonical_mention_obj
                consolidated_pre_dedup_entities.append(canonical_mention_obj)
            
            output.extraction_metadata['initial_entity_mentions_count'] = len(initial_entities)
            output.extraction_metadata['consolidated_entities_before_dedup_count'] = len(consolidated_pre_dedup_entities)
            ...
            for entity in sorted(output.entities, key=lambda e: (-e.confidence, e.entity_id)):
                if entity.confidence < self.confidence_threshold_final: continue
            ...
            final_entities_map_by_id: Dict[str, ExtractedEntity] = {e.entity_id: e for e in output.entities} # These are the ultimate canonical entities

            # Map: any initial entity's original ID -> its cluster_canonical_id
            initial_id_to_cluster_id: Dict[str, str] = {
                e.entity_id: e.canonical_entity_id for e in initial_entities if e.canonical_entity_id
            }
            ...
            for rel in initial_relationships:
                ...
                if not source_cluster_id or not target_cluster_id:
                    self.logger.debug(f"Rel {rel.relationship_id}: could not map source/target original IDs to cluster IDs. Skipping.")
                    continue
                ...
                if source_cluster_id in final_entities_map_by_id and target_cluster_id in final_entities_map_by_id:
                    rel.source_entity_id = source_cluster_id # Use the cluster ID as the link
            ...
            output.extraction_metadata['final_relationships_count'] = len(output.relationships)
            
            # 6. Final Calculations
            output.overall_confidence = self._calculate_overall_confidence(output.entities, output.relationships)
            output.processing_time_sec = round((datetime.now(timezone.utc) - start_time_utc).total_seconds(), 3)
            
            self.logger.info(f"Ontology extraction completed for {document.id} in {output.processing_time_sec}s. "
    ...
    def _calculate_overall_confidence(self, entities: List[ExtractedEntity], 
        ...
        confidences = [e.confidence for e in entities] + [r.confidence for r in relationships]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        status['dependencies_status'] = {
            ...
            'embedding_manager': 'available' if self.embedding_manager else 'unavailable',
            'coreference_model': f"'{self.coref_model_name}' loaded" if self.nlp_coref else f"'{self.coref_model_name}' not loaded/disabled"
        }
