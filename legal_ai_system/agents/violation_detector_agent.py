...
try:
    ...
    from legal_ai_system.core.llm_providers import LLMManager, LLMProviderError, LLMProviderEnum
    from legal_ai_system.core.unified_exceptions import AgentExecutionError  # Fixed class name
    ...
    try:
        ...
        class BaseAgent:
            ...
            def _get_service(self, name):
                return getattr(self.service_container, 'get_service', lambda x: None)(name)
...
class DetectedViolation:
    ...
    violation_id: str = field(default_factory=lambda: f"VIOL_{uuid.uuid4().hex[:8]}") # Unique ID
...
class ViolationDetectorAgent(BaseAgent):
    ...
    def __init__(self, service_container: Any, **config: Any):
        ...
        self.logger.info(f"ViolationDetectorAgentAgent configured with model: {self.llm_config.get('llm_model', 'default')}")
        self.llm_manager: Optional[LLMManager] = self._get_service("llm_manager") # Use helper
        ...
        self.enable_llm_analysis = bool(config.get('enable_llm_analysis', True)) # Renamed from enable_llm_validation
        self.max_text_for_llm = int(config.get('max_text_for_llm', 8000)) # Chars

        self._init_violation_patterns()
        
        self.logger.info(f"{self.name} initialized.", 
                        parameters={'llm_analysis_enabled': self.enable_llm_analysis, 
    ...
    def _init_violation_patterns(self):
        ...
        default_patterns = {
            'Brady Violation': [r"brady\s+violation", r"exculpatory\s+evidence\s+(?:withheld|not\s+disclosed|suppressed)", r"failure\s+to\s+disclose\s+favorable\s+evidence"],
            '4th Amendment Violation': [r"unreasonable\s+search(?:es)?\s+and\s+seizure(?:s)?", r"warrantless\s+search", r"illegal\s+seizure", r"lack\s+of\s+probable\s+cause"],
            '5th Amendment Violation': [r"miranda\s+rights?\s+(?:violation|not\s+read)", r"self[- ]incrimination", r"coerced\s+confession", r"right\s+to\s+remain\s+silent\s+violated"],
            'Law Enforcement Misconduct': [r"police\s+brutality", r"excessive\s+force", r"false\s+arrest", r"fabricated\s+evidence\s+by\s+police", r"perjury\s+by\s+officer"],
            'Evidence Tampering': [r"evidence\s+tampering", r"chain\s+of\s+custody\s+(?:broken|violated|compromised)", r"altered\s+evidence", r"planted\s+evidence"],
            'Witness Tampering': [r"witness\s+intimidation", r"witness\s+tampering", r"coaching\s+witness\s+improperly", r"threatening\s+a\s+witness"]
        }
        # Load from config if available, else use defaults
        self.violation_patterns_config = self.config.get('violation_patterns', default_patterns)
        ...
        for viol_type, patterns in self.violation_patterns_config.items():
            if isinstance(patterns, list) and all(isinstance(p, str) for p in patterns):
                ...
                self.logger.warning(f"Invalid pattern format for violation type '{viol_type}' in config. Skipping.")
        self.logger.debug(f"Initialized {len(self.all_violation_pattern_groups)} violation pattern groups.")
    ...
    async def _process_task(self, task_data: Dict[str, Any], metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Starting violation detection for doc '{document_id}'.", 
        ...
        if not text_content or len(text_content.strip()) < 50:
            self.logger.warning(f"Insufficient text content for violation detection (doc '{document_id}').")
        ...
        try:
            detected_violations_list = await self._detect_all_violations_async(text_content, document_id)
            output.violations_found = self._filter_and_finalize_violations(detected_violations_list)
            
            if output.violations_found:
                output.analysis_summary = await self._analyze_detected_violations(output.violations_found, text_content)
                output.recommendations = await self._generate_recommendations_for_violations(output.violations_found, output.analysis_summary)
            
            if output.violations_found:
                output.overall_confidence = round(sum(v.confidence for v in output.violations_found) / len(output.violations_found), 3)
            else: # No violations found, high confidence in that assessment (assuming thorough check)
                output.overall_confidence = 0.95 

            self.logger.info(f"Violation detection completed for doc '{document_id}'.", 
                            parameters={'violations_found': len(output.violations_found), 'overall_conf': output.overall_confidence})
        
        except AgentProcessingError as ape: # Catch specific agent errors
            self.logger.error(f"AgentProcessingError during violation detection for doc '{document_id}'.", exception=ape, exc_info=True)
            output.errors.append(f"Agent processing error: {str(ape)}")
            output.overall_confidence = 0.1 # Low confidence due to error
        except Exception as e: # Catch unexpected errors
            self.logger.error(f"Unexpected error during violation detection for doc '{document_id}'.", exception=e, exc_info=True)
    ...
    async def _detect_all_violations_async(self, text: str, doc_id: str) -> List[DetectedViolation]:
        ...
        for violation_type_label, regex_patterns in self.all_violation_pattern_groups:
            pattern_matches = self._find_matches_for_patterns(text, regex_patterns, violation_type_label)
            violations.extend(pattern_matches)
        
        self.logger.info(f"Pattern matching found {len(violations)} potential violations for doc '{doc_id}'.")
        ...
        if self.enable_llm_analysis and self.llm_manager:
            try:
                ...
                if len(text) > self.max_text_for_llm:
                    self.logger.warning(f"Text for doc '{doc_id}' truncated to {self.max_text_for_llm} chars for LLM analysis.")

                llm_detected_violations = await self._llm_analyze_text_for_violations(text_for_llm, violations, doc_id)
                violations = self._merge_llm_and_pattern_violations(violations, llm_detected_violations)
                self.logger.info(f"LLM analysis resulted in {len(violations)} total potential violations for doc '{doc_id}'.")
            except Exception as llm_e: # Catch errors specifically from the LLM step
                self.logger.error(f"LLM violation analysis failed for doc '{doc_id}'. Pattern results will be used.", exception=llm_e)
                # `violations` will still contain pattern-based results.
        
        return violations
    
    def _find_matches_for_patterns(self, text: str, patterns_list: List[str], violation_type: str) -> List[DetectedViolation]:
        ...
        for regex_str_pattern in patterns_list:
            try:
                for match_obj in re.finditer(regex_str_pattern, text, re.IGNORECASE | re.DOTALL):
                    ...
                    context_text = text[max(0, start_idx - 250) : min(len(text), end_idx + 250)].strip()
                    matched_text_snippet = match_obj.group(0).strip()
                    
                    confidence_val = self._calculate_pattern_match_confidence(regex_str_pattern, matched_text_snippet)
                    
                    if confidence_val >= self.min_pattern_confidence:
                        matches_found.append(DetectedViolation(
                            violation_type=violation_type,
                            description=matched_text_snippet,
                            context=context_text,
                            confidence=confidence_val,
                            severity=self._assess_violation_severity(violation_type, matched_text_snippet),
                            start_pos=start_idx,
                            end_pos=end_idx,
                 ...
                 self.logger.warning(f"Regex error during violation pattern matching for type '{violation_type}'.", 
                                   parameters={'pattern': regex_str_pattern, 'error': str(re_err)})
        return matches_found

    def _calculate_pattern_match_confidence(self, pattern_str: str, matched_text: str) -> float:
        base_conf = 0.55 
        if len(pattern_str) > 20: base_conf += 0.1 # Longer patterns are more specific
        if len(matched_text.split()) > 3: base_conf += 0.1 # Multi-word matches are better
        
        critical_terms = ['brady', 'miranda', 'unconstitutional', 'tampering', 'perjury', 'fabricated', 'coerced']
        if any(term in matched_text.lower() for term in critical_terms): 
            base_conf += 0.2
        
        return round(min(0.95, base_conf), 3)

    def _assess_violation_severity(self, violation_type_label: str, description_text: str) -> str:
        desc_lower = description_text.lower()
        type_lower = violation_type_label.lower()

        if any(kw in desc_lower for kw in ['fabricated evidence', 'planted evidence', 'destroyed evidence', 'coerced confession', 'perjury by officer', 'witness intimidation by force']): return 'critical'
        if 'brady' in type_lower or 'constitutional' in type_lower or '4th amendment' in type_lower or '5th amendment' in type_lower or \
           any(kw in desc_lower for kw in ['suppressed evidence', 'evidence withheld', 'false testimony']): return 'high'
        if 'misconduct' in type_lower or 'tampering' in type_lower or \
           any(kw in desc_lower for kw in ['excessive force', 'false arrest', 'improper influence']): return 'medium'
        return 'low'

    async def _llm_analyze_text_for_violations(self, text_for_llm: str, current_violations: List[DetectedViolation], doc_id: str) -> List[DetectedViolation]:
        if not self.llm_manager: return []
        self.logger.debug(f"Using LLM for violation analysis and discovery on doc '{doc_id}'.")

        # Summarize existing pattern-based findings for the LLM prompt
        pattern_findings_summary = "Potential Violations (from patterns, to be validated by you):\n"
        if current_violations:
            ...
            pattern_findings_summary = "No violations found by initial pattern matching."
        ...
        """
        model_to_use = self.config.get('llm_model_for_violation_detection', self.llm_manager.primary_config.model)
        provider_to_use = self.config.get('llm_provider_for_violation_detection', self.llm_manager.primary_config.provider.value)

        self.logger.debug(f"Sending violation analysis prompt to LLM ({provider_to_use}/{model_to_use}) for doc '{doc_id}'.")
        ...
        try:
            ...
            llm_response = await self.llm_manager.complete(
                prompt,
                model=model_to_use,
                provider=LLMProviderEnum(provider_to_use), # Ensure it's enum if manager expects it
                model_params={'temperature': 0.1, 'max_tokens': 2000}
            )
            output.model_used_for_llm = f"{provider_to_use}/{model_to_use}" # Store which model was used
            return self._parse_llm_violation_response(llm_response.content, text_for_llm, doc_id)
        except LLMProviderError as e:
            self.logger.error(f"LLM API call for violation analysis failed for doc '{doc_id}'.", exception=e)
            raise AgentProcessingError("LLM analysis failed.", underlying_exception=e) from e
        except Exception as e:
            self.logger.error(f"Unexpected error in LLM violation analysis for doc '{doc_id}'.", exception=e)
            raise AgentProcessingError("Unexpected error during LLM processing.", underlying_exception=e) from e

    def _parse_llm_violation_response(self, response_content: str, original_text: str, doc_id: str) -> List[DetectedViolation]:
        llm_violations: List[DetectedViolation] = []
        try:
            # Robust JSON extraction from LLM response
            json_match = re.search(r'\[\s*(\{[\s\S]*?\})\s*\]', response_content, re.DOTALL) # Try to find array of objects
            if not json_match:
                json_match_single = re.search(r'(\{[\s\S]*?\})', response_content, re.DOTALL) # Try to find single object if array fails
                if json_match_single:
                    json_str = f"[{json_match_single.group(1)}]" # Wrap single object in array
                else:
                    self.logger.warning(f"No JSON array or object found in LLM violation response for doc '{doc_id}'. Response start: {response_content[:150]}")
                    return []
            else:
                json_str = json_match.group(0) # The full array string
            
            violations_data_list = json.loads(json_str)
            if not isinstance(violations_data_list, list): # Ensure it's a list after parsing
                 self.logger.warning(f"Parsed LLM violation response is not a list for doc '{doc_id}'. Found: {type(violations_data_list)}")
                 return []
            ...
            for item_data in violations_data_list:
                ...
                if confidence_val < self.min_llm_confidence: continue
                ...
                if exact_quote:
                    try:
                        match_quote = re.search(re.escape(exact_quote[:100]), original_text) # Match first 100 chars for robustness
                        if match_quote:
                            start_pos, end_pos = match_quote.span()
                    except re.error:
                        self.logger.warning(f"Regex error finding exact quote for LLM violation on doc '{doc_id}'. Quote: {exact_quote[:50]}")

                violation = DetectedViolation(
                    violation_id=item_data.get('violation_id', self._generate_unique_id("LLM_VIOL")),
                    violation_type=item_data.get('violation_type', 'Unknown LLM Violation'),
                    description=description if description else exact_quote, # Prefer description, fallback to quote
                    context=exact_quote if exact_quote else description[:250], # Use quote as context if available
                    confidence=confidence_val,
            ...
            self.logger.error(f"Failed to parse JSON from LLM violation response for doc '{doc_id}'. JSON string was: '{json_str if 'json_str' in locals() else response_content[:200]}'", exception=e)
        except Exception as e: # Catch other errors during parsing individual items
            self.logger.error(f"Unexpected error parsing an LLM violation item for doc '{doc_id}'. Item data: {item_data if 'item_data' in locals() else 'N/A'}", exception=e)
        
        self.logger.debug(f"LLM response parsing yielded {len(llm_violations)} violations for doc '{doc_id}'.")
    ...
    def _merge_llm_and_pattern_violations(self, pattern_violations: List[DetectedViolation], 
        ...
        if not llm_violations: return pattern_violations # No LLM results, keep pattern ones
        if not pattern_violations: return llm_violations # No pattern results, use LLM ones
        ...
        for lv in llm_violations:
            key_desc = re.sub(r'\s+', ' ', lv.description.lower().strip()[:60]) # Normalized first 60 chars
        ...
        for pv in pattern_violations:
            ...
            for (llm_type_key, llm_desc_key), llm_viol_obj in llm_map.items():
                ...
                if llm_type_key == key_pv[0] and (key_pv[1] in llm_desc_key or llm_desc_key in key_pv[1]):
                    ...
                    if pv.confidence > llm_viol_obj.confidence: # Example: take higher confidence
                         llm_viol_obj.confidence = pv.confidence
                    if pv.start_pos is not None and llm_viol_obj.start_pos is None: # Prefer pattern span if LLM lacks one
    ...
    def _filter_and_finalize_violations(self, violations: List[DetectedViolation]) -> List[DetectedViolation]:
        ...
        confident_violations = [v for v in violations if v.confidence >= self.min_llm_confidence or 
                                (v.detected_by == 'pattern_matching' and v.confidence >= self.min_pattern_confidence)]
        
        if not confident_violations: return []
        ...
        confident_violations.sort(key=lambda v: (-v.confidence, v.start_pos if v.start_pos is not None else float('inf')))
        ...
        for viol in confident_violations:
            ...
            for existing_viol in finalized_list:
                ...
                if viol.violation_type == existing_viol.violation_type and \
                   viol.start_pos is not None and viol.end_pos is not None and \
                   existing_viol.start_pos is not None and existing_viol.end_pos is not None:
                    ...
                    if overlap_end > overlap_start: # They overlap
                        ...
                        min_len = min(viol.end_pos - viol.start_pos, existing_viol.end_pos - existing_viol.start_pos)
                        if min_len > 0 and (overlap_len / min_len) > 0.7:
                            ...
                            self.logger.trace(f"Deduplicating violation '{viol.violation_id}' due to overlap with '{existing_viol.violation_id}'.")
                            break
            if not is_duplicate:
    ...
    async def _analyze_detected_violations(self, violations: List[DetectedViolation], text_content: str) -> Dict[str, Any]:
        ...
        potential_impacts = ["Review detected violations for specific case impact."] # Default
        critical_count = severity_summary.get('critical', 0)
        high_count = severity_summary.get('high', 0)

        if critical_count > 0:
            potential_impacts = [f"CRITICAL: {critical_count} violation(s) may warrant immediate action (e.g., motion to dismiss, sanctions)."]
        elif high_count > 0:
            potential_impacts = [f"HIGH: {high_count} violation(s) suggest significant legal challenges, potential evidence suppression, or grounds for appeal."]
    ...
    async def _generate_recommendations_for_violations(self, violations: List[DetectedViolation], 
        ...
        if not violations: return recommendations
        ...
        unique_violation_types = analysis_summary.get("violation_types_summary", {}).keys()
        for v_type in unique_violation_types:
            if "Brady" in v_type: 
                recommendations.append({'priority': 'HIGH', 'action': 'Scrutinize all discovery for withheld exculpatory or impeachment evidence. File Brady motion if applicable.', 'reasoning': f'Potential {v_type}.'})
            elif "4th Amendment" in v_type: 
                recommendations.append({'priority': 'HIGH', 'action': 'Evaluate legality of search/seizure. Consider motion to suppress resulting evidence.', 'reasoning': f'Potential {v_type}.'})
            elif "5th Amendment" in v_type:
                recommendations.append({'priority': 'HIGH', 'action': 'Assess circumstances of any statements/confessions for coercion or Miranda violations.', 'reasoning': f'Potential {v_type}.'})
            elif "Evidence Tampering" in v_type:
                recommendations.append({'priority': 'CRITICAL', 'action': 'Investigate chain of custody and evidence integrity. This could be grounds for severe sanctions or case dismissal.', 'reasoning': f'Potential {v_type}.'})

        if not recommendations and violations: # Generic if no specific high-priority ones triggered
            recommendations.append({
                'priority': 'MEDIUM', 
