    ...
    class CircuitBreaker:  # type: ignore
        ...
        def _is_open(self) -> bool:
            ...
            if self._last_failure is None:
                return False
            if (
                datetime.now() - self._last_failure
...
class KnowledgeBaseAgent(BaseAgent):
    ...
    def __init__(self, services):
        ...
        logger.info(
            "KnowledgeBase agent initialized with design.txt compliance",
    ...
    async def _process_task(self, task_data, metadata):
        ...
        logger.info(
            "knowledge_base_processing_started",
        ...
        try:
            ...
            organizational_structure = await self._create_organizational_structure(
                resolved_entities
            )
            ...
            result = EntityResolutionResult(
                ...
                resolution_metrics=self._calculate_resolution_metrics(
                    raw_entities, resolved_entities
                ),
            ...
            logger.info(
                "knowledge_base_processing_completed",
                document_id=document_id,
            ...
            logger.error(
                "knowledge_base_processing_failed",
                document_id=document_id,
                error=str(e),
    ...
    def _generate_cache_key(self, entities: List[Dict], metadata: Dict) -> str:
        ...
        entity_hash = hashlib.md5(
            json.dumps(entity_names, sort_keys=True).encode()
        ).hexdigest()
    ...
    async def _resolve_entities_with_protection(
        self, raw_entities: List[Dict], document_id: str
    ) -> List[ResolvedEntity]:
    ...
    @knowledge_base_breaker
    async def _persist_entities_to_graph(
        self, entities: List[ResolvedEntity], document_id: str
    ) -> None:
    ...
    async def _create_organizational_structure(
        ...
        structure["metadata"]["entity_types"] = list(
            structure["metadata"]["entity_types"]
        )
    ...
    async def _generate_analytical_insights(
        self, entities: List[ResolvedEntity], metadata: Dict
    ) -> Dict[str, Any]:
        ...
        if entities:
            ...
            insights["quality_metrics"]["overall_confidence"] = sum(
                e.confidence_score for e in entities
            ) / len(entities)

            # Find most confident entities
            sorted_entities = sorted(
                entities, key=lambda x: x.confidence_score, reverse=True
            )
    ...
    def _calculate_resolution_metrics(
        self, raw_entities: List[Dict], resolved_entities: List[ResolvedEntity]
    ) -> Dict[str, float]:
        ...
        return {
            ...
            "deduplication_ratio": (
                (len(raw_entities) - len(resolved_entities)) / len(raw_entities)
                if raw_entities
                else 0
            ),
            "high_confidence_entities": sum(
                1 for e in resolved_entities if e.confidence_score >= 0.8
            ),
            "entities_with_aliases": sum(1 for e in resolved_entities if e.aliases),
            "multi_source_entities": sum(
                1 for e in resolved_entities if len(e.source_documents) > 1
            ),
        }
    ...
    def _update_metrics(self, result: EntityResolutionResult) -> None:
        ...
        self.metrics["entities_processed"] += result.resolution_metrics.get(
            "input_entities", 0
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            circuit_breaker_status = (
                "closed"
                if not knowledge_base_breaker.current_state
                else str(knowledge_base_breaker.current_state)
            )
            ...
            health_status = {
                ...
                "status": (
                    "healthy"
                    if all([components_healthy, storage_healthy])
                    else "degraded"
                ),
                "components": {
                    ...
                    "shared_components": (
                        "healthy" if components_healthy else "unhealthy"
                    ),
                ...
                "storage_stats": {
                    ...
                    "entities_with_aliases": sum(
                        1 for e in self.entity_registry.values() if e.aliases
                    ),
                    "multi_source_entities": sum(
                        1
                        for e in self.entity_registry.values()
