...
agent_mem_logger = get_detailed_logger("UnifiedMemoryManager.AgentMemory", LogCategory.DATABASE)
session_mem_logger = get_detailed_logger("UnifiedMemoryManager.SessionMemory", LogCategory.DATABASE)
context_mem_logger = get_detailed_logger("UnifiedMemoryManager.ContextWindowMemory", LogCategory.DATABASE)
...
class UnifiedMemoryManager:
    ...
    @detailed_log_function(LogCategory.DATABASE)
    async def add_session_observation(self, session_id: str, entity_id: str, content: str,
                                    importance: float = 0.5, source: str = "system") -> str:
        session_mem_logger.info("Adding session observation.", parameters={'session': session_id, 'entity': entity_id})
        ...
        def _add_sync():
            with self._lock, self._get_db_connection() as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (observation_id, entity_id, session_id, content, importance, source, now_iso))
                conn.commit()
        try:
            await asyncio.get_event_loop().run_in_executor(None, _add_sync)
            session_mem_logger.debug("Session observation added.", parameters={'id': observation_id})
    ...
    @detailed_log_function(LogCategory.DATABASE)
    async def create_session_relationship(self, session_id: str, source_entity_id: str, 
        ...
        def _create_sync():
            with self._lock, self._get_db_connection() as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (relationship_id, session_id, source_entity_id, target_entity_id, 
                      relationship_type, props_json, confidence, now_iso))
                conn.commit()
        try:
            await asyncio.get_event_loop().run_in_executor(None, _create_sync)
            session_mem_logger.debug("Session relationship created.", parameters={'id': relationship_id})
    ...
    @detailed_log_function(LogCategory.DATABASE)
    async def get_session_entity(self, session_id: str, entity_type: str, name: str) -> Optional[Dict[str, Any]]:
        session_mem_logger.debug("Getting session entity.", 
        ...
        def _get_sync():
            with self._lock, self._get_db_connection() as conn:
                ...
                if row:
                    ...
                    entity_dict['attributes'] = json.loads(entity_dict['attributes']) if entity_dict['attributes'] else {}
                    return entity_dict
                return None
        try:
            entity = await asyncio.get_event_loop().run_in_executor(None, _get_sync)
            if entity:
                session_mem_logger.debug("Session entity found.", parameters={'name': name})
            else:
                session_mem_logger.debug("Session entity not found.", parameters={'name': name})
            return entity
        except Exception as e:
    ...
    @detailed_log_function(LogCategory.DATABASE)
    async def get_session_entities(self, session_id: str, entity_type: Optional[str] = None) -> List[Dict[str, Any]]:
        session_mem_logger.debug("Getting session entities.", 
        ...
        def _prune_sync():
            with self._lock, self._get_db_connection() as conn:
                ...
                for row in all_entries:
                    entry_token_count = row['token_count'] if row['token_count'] is not None else 50
    ...
    # --- Agent Decisions Log ---
    @detailed_log_function(LogCategory.DATABASE)
    async def log_agent_decision(self, agent_name: str, session_id: Optional[str], 
                               input_summary: str, decision_details: Dict[str, Any],
                               context_used: Optional[Dict[str, Any]] = None,
                               confidence: float = 1.0,
                               tags: Optional[List[str]] = None) -> str:
        agent_mem_logger.info("Logging agent decision.", parameters={'agent': agent_name, 'session': session_id})
        ...
        def _log_sync():
            with self._lock, self._get_db_connection() as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (decision_id, agent_name, session_id, input_summary, decision_json, 
                      context_json, confidence, tags_json, created_at_iso))
                conn.commit()
        try:
            await asyncio.get_event_loop().run_in_executor(None, _log_sync)
            agent_mem_logger.debug("Agent decision logged.", parameters={'id': decision_id})
            return decision_id
        except Exception as e:
    ...
    @detailed_log_function(LogCategory.DATABASE)
    async def get_agent_decisions(self, agent_name: str, session_id: Optional[str] = None,
                                limit: int = 10) -> List[Dict[str, Any]]:
        agent_mem_logger.debug("Getting agent decisions.", 
        ...
        def _get_sync():
            with self._lock, self._get_db_connection() as conn:
                ...
                for row in cursor:
                    ...
                    decision_dict['decision_details'] = json.loads(decision_dict['decision_details'])
                    decision_dict['context_used'] = json.loads(decision_dict['context_used']) if decision_dict['context_used'] else {}
                    decision_dict['tags'] = json.loads(decision_dict['tags']) if decision_dict['tags'] else []
    ...
    @detailed_log_function(LogCategory.DATABASE)
    async def get_pattern_stats_async(self, pattern_regex: str) -> Optional[Dict[str, Any]]:
        self._record_op("get_pattern_stats")
        pattern_hash = hashlib.md5(pattern_regex.encode()).hexdigest()
        def _get_sync():
            with self._lock, self._get_db_connection() as conn:
                conn.row_factory = sqlite3.Row
                row = conn.execute("SELECT * FROM pattern_learning_stats WHERE pattern_hash = ?", (pattern_hash,)).fetchone()
                return dict(row) if row else None
        try:
            return await asyncio.get_event_loop().run_in_executor(None, _get_sync)
        except Exception as e:
            umm_logger.error("Failed to get pattern stats.", parameters={'pattern': pattern_regex}, exception=e)
            return None

    @detailed_log_function(LogCategory.DATABASE)
    async def update_pattern_stats_async(self, pattern_regex: str, 
                                       effectiveness_score: Optional[float] = None,
                                       usage_increment: int = 1) -> None:
                ...
                if effectiveness_score is not None:
                    conn.execute("""
                        ...
                        ON CONFLICT(pattern_hash) DO UPDATE SET
                            ...
                            last_updated = excluded.last_updated;
                    """, (pattern_hash, pattern_regex, effectiveness_score, usage_increment, now_iso))
                else:
                    conn.execute("""
                        INSERT INTO pattern_learning_stats 
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(None, _update_sync)
            umm_logger.trace("Pattern stats updated.", parameters={'pattern': pattern_regex})
        except Exception as e:
            umm_logger.error("Failed to update pattern stats.", parameters={'pattern': pattern_regex}, exception=e)
        ...
        def _get_stats_sync():
            ...
            try:
                with self._get_db_connection() as conn:
                    ...
                    stats['tag_learning_stats_entries'] = conn.execute("SELECT COUNT(*) FROM tag_learning_stats").fetchone()[0]
                    stats['pattern_learning_stats_entries'] = conn.execute("SELECT COUNT(*) FROM pattern_learning_stats").fetchone()[0]
