...
class MLOptimizer:
    ...
    def __init__(self, storage_dir: str = "./storage/databases", service_config: Optional[Dict[str, Any]] = None):
        ...
        self.max_optimization_age_hours = self.config.get('max_optimization_age_hours', 24)
        ...
        ml_logger.info("ML optimizer initialization complete", parameters={
    ...
    def _load_recent_performance(self):
        ...
        try:
            ...
            ml_logger.info(f"Loaded {len(self.performance_history)} recent performance records")
            
        except Exception as e:
            ml_logger.error("Failed to load recent performance data", exception=e)
    
    @detailed_log_function(LogCategory.PERFORMANCE)
    def record_performance(self, document_path: str, document_type: DocumentCategory, 
                          parameters: ProcessingParameters, metrics: PerformanceMetrics,
                          features: DocumentFeatures, objective: OptimizationObjective = OptimizationObjective.BALANCED):
        ...
        try:
            with self._lock:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        document_path, document_type.value, doc_hash,
                        json.dumps(parameters.to_dict()),
                        json.dumps(asdict(metrics)),
                        json.dumps(asdict(features)),
                        objective.value, composite_score
                    ))
                ...
                performance_logger.info(f"Performance recorded", parameters={
                    'document_type': document_type.value,
                    'composite_score': composite_score,
            ...
            performance_logger.error(f"Failed to record performance for {document_path}", exception=e)
    
    @detailed_log_function(LogCategory.SYSTEM)
    async def get_optimal_parameters(self, document_type: DocumentCategory, 
                                   document_features: DocumentFeatures,
                                   objective: OptimizationObjective = OptimizationObjective.BALANCED,
                                   force_refresh: bool = False) -> OptimizationResult:
        """Get ML-optimized processing parameters for document."""
        
        optimization_logger.info(f"Getting optimal parameters for {document_type.value}")
        
        # Check cache first
        cache_key = self._generate_cache_key(document_type, document_features, objective)
        ...
        if not force_refresh:
            ...
            if db_result:
                ...
                optimization_logger.info("Returning database cached optimization result")
                return db_result
        
        # Find similar documents
        similar_docs = self._find_similar_documents(document_type, document_features, objective)
        
        if len(similar_docs) < self.min_samples_for_optimization:
            optimization_logger.warning(f"Insufficient samples for optimization: {len(similar_docs)} < {self.min_samples_for_optimization}")
            return self._get_default_optimization_result(document_type, objective)
        
        # Analyze best performing parameters
        optimal_params, improvement, confidence, reason = self._optimize_parameters(similar_docs, objective)
        ...
        optimization_logger.info(f"Optimization complete", parameters={
            'document_type': document_type.value,
    ...
    def _calculate_document_hash(self, document_path: str, features: DocumentFeatures) -> str:
    ...
    def _find_similar_documents(self, document_type: DocumentCategory, 
                               document_features: DocumentFeatures,
                               objective: OptimizationObjective) -> List[Dict[str, Any]]:
        ...
        for record in self.performance_history:
            ...
            if (record['document_type'] != document_type.value or 
        ...
        similar_docs.sort(key=lambda x: (x['similarity'], x['composite_score']), reverse=True)
        
        optimization_logger.trace(f"Found {len(similar_docs)} similar documents", parameters={
            'document_type': document_type.value,
            'similarity_threshold': self.similarity_threshold
        })
    ...
    def _optimize_parameters(self, similar_docs: List[Dict[str, Any]], 
                           objective: OptimizationObjective) -> Tuple[ProcessingParameters, float, float, str]:
        """Optimize parameters based on similar documents performance."""
        
        # Sort by performance score
        sorted_docs = sorted(similar_docs, key=lambda x: x['composite_score'], reverse=True)
        ...
        avg_score = sum(doc['composite_score'] for doc in similar_docs) / len(similar_docs)
        expected_improvement = (top_score - avg_score) / max(0.01, avg_score)
        
        # Calculate confidence based on sample size and score variance
        score_variance = np.var([doc['composite_score'] for doc in similar_docs])
    ...
    def _generate_cache_key(self, document_type: DocumentCategory, 
                           document_features: DocumentFeatures,
                           objective: OptimizationObjective) -> str:
        """Generate cache key for optimization results."""
        feature_hash = hashlib.md5(str(document_features.to_vector()).encode()).hexdigest()[:8]
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    return OptimizationResult(
                        optimized_parameters=ProcessingParameters.from_dict(json.loads(row[0])),
    ...
    def _cache_optimization_result(self, cache_key: str, result: OptimizationResult):
        ...
        try:
            expires_at = datetime.now() + timedelta(hours=self.max_optimization_age_hours)
            ...
            optimization_logger.error("Failed to cache optimization result", exception=e)
    
    def _get_default_optimization_result(self, document_type: DocumentCategory, 
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        if self.performance_history:
            recent_params = [record['parameters'] for record in list(self.performance_history)[-100:]]
            
            for param_name in ['chunk_size', 'temperature', 'confidence_threshold']:
                values = [p.get(param_name) for p in recent_params if p.get(param_name) is not None]
                if values:
